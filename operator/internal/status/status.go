...
func Refresh(ctx context.Context, k k8s.Client, req ctrl.Request, now time.Time, statusInfo *LokiStackStatusInfo, degradedErr *DegradedError) error {
	...
	statusUpdater := func(stack *lokiv1.LokiStack) {
		...
		stack.Status.NetworkPolicies = statusInfo.NetworkPolicy
	}
