...
func TestBuildAll_WithFeatureFlags_EnableNonRootUser(t *testing.T) {
	type test struct {
		desc         string
		BuildOptions Options
	}
	...
	table := []test{
		{
			...
			BuildOptions: Options{
				...
				Flags: FeatureFlags{
					EnableNonRootUser: false,
				},
			},
		},
		{
			desc: "set nonroot user",
			BuildOptions: Options{
				Name:      "test",
				Namespace: "test",
				Stack: lokiv1beta1.LokiStackSpec{
	...
	for _, tst := range table {
		...
		t.Run(tst.desc, func(t *testing.T) {
			...
			for _, o := range objs {
				...
				t.Run(name, func(t *testing.T) {
					if tst.BuildOptions.Flags.EnableNonRootUser {
						require.NotNil(t, spec.SecurityContext.RunAsUser)
						require.NotZero(t, spec.SecurityContext.RunAsUser)
					} else {
						require.Nil(t, spec.SecurityContext.RunAsUser)
					}
				})
			}
		})
