		...
		Containers: []corev1.Container{
			{
				...
				Args: []string{
					...
					fmt.Sprintf("--logs.write-timeout=%s", opts.Server.HTTP.GatewayUpstreamWriteTimeout),
					...
					fmt.Sprintf("--server.read-timeout=%s", opts.Server.HTTP.GatewayReadTimeout),
					fmt.Sprintf("--server.write-timeout=%s", opts.Server.HTTP.GatewayWriteTimeout),
				},
