...
func TestBuildIngester_PodDisruptionBudgetWithCustomReplicationFactor(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			Name:                 "Extra small stack, no errors",
		...
		{
			Name:     "Pico stack, RF > replicas",
			Size:     lokiv1.SizeOneXPico,
			Replicas: 3,
			RF:       4,
			ExpectedError: &status.DegradedError{
				Message: "Invalid configuration: ingester replicas (3) should be more than the replication factor (4)",
				Reason:  lokiv1.ReasonInvalidReplicationFactor,
				Requeue: true,
			},
		},
		{
			Name:     "Extra small stack, RF > replicas",
			Size:     lokiv1.SizeOneXSmall,
			Replicas: 2,
			RF:       3,
			ExpectedError: &status.DegradedError{
				Message: "Invalid configuration: ingester replicas (2) should be more than the replication factor (3)",
				Reason:  lokiv1.ReasonInvalidReplicationFactor,
				Requeue: true,
			},
		},
		{
			Name:                 "Small stack, RF == replicas",
			Size:                 lokiv1.SizeOneXSmall,
			ExpectedMinAvailable: 1,
			Replicas:             2,
			RF:                   2,
		...
		{
			Name:                 "Medium stack, RF > replicas",
			Size:                 lokiv1.SizeOneXMedium,
			ExpectedMinAvailable: 2,
			Replicas:             3,
			RF:                   5,
			ExpectedError: &status.DegradedError{
				Message: "Invalid configuration: ingester replicas (3) should be more than the replication factor (5)",
				Reason:  lokiv1.ReasonInvalidReplicationFactor,
				Requeue: true,
			},
		},
		...
		t.Run(tc.Name, func(t *testing.T) {
			...
			if tc.ExpectedError != nil {
				...
				require.NotNil(t, opts.ResourceRequirements.Ingester.PDBMinAvailable)
				require.Equal(t, tc.ExpectedMinAvailable, opts.ResourceRequirements.Ingester.PDBMinAvailable)
			}
		})
	}
}
