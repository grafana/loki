...
Extracting data (for use by other stages)
  ...
  * [logfmt](#logfmt) - parse Splunk or logfmt lgo and extract data, timestamp and line as logfmt
...
A logfmt stage will parse the logfile and return the `key=value` pairs in the `extracted` map, parse the timestamp and convert Splunk loglines to logfmt.
...
Logline: `ts=2019-06-14T12:45:43+00:00, lvl=debug, app=app, int=1234, float=1.337 msg="test log message"`

Would create the follwing `extracted` map:
...
And following timestamp:

```go
time.Date(2019, 06, 14, 12, 45, 43, 0, time.FixedZone("UTC", 0))
```

And following logline (cleaned to logfmt):

```
ts=2019-06-14T12:45:43+00:00 lvl=debug app=app int=1234 float=1.337 msg="test log message"
```

[Example in unit test](../../pkg/logentry/stages/logfmt_test.go)
