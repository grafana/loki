...
# Maximum size of log query responses that can be cached. Responses larger than
# this will not be cached. 0 means unlimited (all responses will be cached
# regardless of size).
# CLI flag: -querier.log-result-cache-max-response-size
[log_result_cache_max_response_size: <int> | default = 0B]

# Enable caching of non-empty log query responses. When disabled, only empty
