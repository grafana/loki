...
- Update the value of `shared_store` to the type of object storage you are using. Options are `gcs`, `s3`, `azure`, `cos` or `filesystem`. Update the `object_store` variable under the `schema_config` section to the same value. 
...
loki + promtail + gateway {
  _config+:: {
    ...
    shared_store: 'my-object-storage-backend-type',
