...
### Deferred OTLP structured metadata expansion

Loki now supports the experimental per-tenant limit
`otlp_defer_structured_metadata_expansion`. When enabled, OTLP resource and scope
attributes are carried once per stream instead of being copied into the structured
metadata of every log entry, and are expanded only when entries are written to
storage. This reduces write-path byte amplification for OTLP tenants.
