...
1. **Configuration:**: We do not account for all configuration paramters in this guide. We only cover the parameters that need to be changed. Other paramters can remain the same. **However:** if `pattern_ingesters=true` you will need to spin up `patternIngesters` before shutting down the SSD ingesters. This is primarly needed for the Grafana drilldown logs feature.
