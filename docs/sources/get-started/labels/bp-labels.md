...
If you often extract a label from a log line at query time, the label has a high cardinality, and extracting that label is expensive in terms of performance; consider extracting the label on the client side and
attach it as metadata to log lines using a [non-indexed label]({{< relref "./non-indexed-labels" >}}).
