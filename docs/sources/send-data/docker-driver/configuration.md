...
| `no-file`                       |    No     |          `false`*          | Default is "false" and files are created.  Set to "true" to tell driver not to ever create log files on disk. However, this means you won't be able to use `docker logs` on the container anymore. Use this setting if you don't need to use `docker logs` or run with limited disk space.                      |
