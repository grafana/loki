...
### Sending samples to Grafana Mimir

Grafana Mimir accepts samples on its [remote-write endpoint](https://grafana.com/docs/mimir/<MIMIR_VERSION>/references/http-api/#remote-write)
at `/api/v1/push`. Recording rule samples generated by Loki's ruler can be forwarded to Mimir by pointing a remote-write client at that endpoint.

#### Tenant handling

In a multi-tenant Loki deployment, the ruler runs each tenant's rules independently and labels every outbound remote-write
request with that tenant's ID. The `add_org_id_header` option (enabled by default) controls how Loki communicates the tenant
to the remote-write target:

- **`add_org_id_header: true` (default)**: Loki injects an `X-Scope-OrgID` header on each request, set to the ID of the
  Loki tenant whose rules produced the samples. Mimir uses this same header to identify the destination Mimir tenant, so
  the configuration yields a 1:1 mapping from Loki tenant to Mimir tenant.
- **`add_org_id_header: false`**: Loki does not send `X-Scope-OrgID`. Use this when forwarding to a single-tenant Mimir
  deployment, or when an upstream component (for example, a proxy or `nginx-ingress` running [auth-enabled Mimir](https://grafana.com/docs/mimir/<MIMIR_VERSION>/manage/secure/authentication-and-authorization/)) sets the tenant header itself.

{{< admonition type="note" >}}
Loki strips any `X-Scope-OrgID` value (in any letter case) that you set under a client's `headers:` block before sending
the request. This prevents a misconfiguration from accidentally writing one Loki tenant's samples into another Mimir
tenant. To control the Mimir tenant ID, use `add_org_id_header` or perform the mapping on a proxy between Loki and Mimir.
{{< /admonition >}}

#### Multi-tenant example

The following ruler configuration sends recording rule samples from every Loki tenant to the matching Mimir tenant:
  ...
  remote_write:
    ...
    clients:
      mimir:
        ...
        # X-Scope-OrgID is injected automatically per Loki tenant; do not set it here.
        write_relabel_configs:
          - action: replace
            target_label: job
            replacement: loki-recording-rules
...
#### Single-tenant or basic-auth example

To forward all Loki rule samples to a single Mimir tenant — for example, a self-hosted Mimir with authentication
terminated at a reverse proxy, or [Grafana Cloud Metrics](https://grafana.com/docs/grafana-cloud/send-data/metrics/metrics-prometheus/) — disable the org-ID header and set credentials directly on the client:
...
Refer to the [Grafana Cloud Metrics endpoint documentation](https://grafana.com/docs/grafana-cloud/send-data/metrics/metrics-prometheus/) for the exact URL and credentials for your stack.

#### Verifying samples reach Mimir

To confirm the ruler is delivering samples successfully:

- The metric `loki_ruler_wal_prometheus_remote_storage_samples_total{tenant="<tenant-id>"}` increases over time.
- The lag between
  `loki_ruler_wal_prometheus_remote_storage_highest_timestamp_in_seconds` and
  `loki_ruler_wal_prometheus_remote_storage_queue_highest_sent_timestamp_seconds`
  stays small (see [Remote-Write Lagging](#remote-write-lagging) below).
- Query the recording rule's metric name in Mimir (set the tenant ID to match the Loki tenant when running a multi-tenant Mimir).
