...
All samples generated by recording rules are written to a WAL. A WAL is created per tenant, and this is done to maximize
durability and minimize noisy-neighbour interactions. We are trading off extra memory usage and slower start-up times
for this functionality.

Prometheus will, for example, reject a request with 100 samples if just 1 of those samples is invalid in some way, so we
don't want one tenant's bad samples to affect another's from being ingested into Prometheus.
