...
`KB` and `#entries` are Loki additions used by the query frontend to plan [dynamic query sharding](../../tsdb/#dynamic-query-sharding). Because the metas form a single delta chain, computing statistics for a time range requires decoding chunks from the start of the chain. [Version 3](../v3/) removes this cost by adding page markers.
