...
Version 3 is written for periods with schema version `v13`. It is identical to [version 2](../v2/) except for the chunks part of a series entry, which gains page markers so that readers can skip over chunks that fall outside the queried time range. See [TSDB index format](../) for the encoding notation used on this page.
