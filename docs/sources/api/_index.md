...
* `max_query_range=<duration>`: The maximum time period the delete request can span. If the request is larger than this value, is it split into several requests of <= `max_query_range`. Valid time units are "s", "m", "h".
