...
- Snippets: These are code snippets illustrating how to configure a specific feature. They are not ready to run and require additional configuration.
...
Additionally, you can run the following command to verify that the example is valid. Note that validation is only run for examples, not snippets.
```bash
