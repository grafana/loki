...
**Note: The distributor uses the `ring` component under the hood to register itself amongst it's peers and get the total number of active distributors. This is a different "key" than the ingesters use in the ring and comes from the distributor's own [ring configuration]({{<relref "../../../configure/#distributor">}}).**
...
Loki can be configured to [accept out-of-order writes]({{<relref "../../../configure/#accept-out-of-order-writes">}}).
