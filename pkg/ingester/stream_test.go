	...
	for _, chk := range []struct {
		...
		t.Run(chk.name, func(t *testing.T) {
			...
			for i := 0; i < 100; i++ {
				...
				iter, err := s.Iterator(context.TODO(), time.Unix(int64(from), 0), time.Unix(int64(from+len), 0), logproto.FORWARD, nil, logql.NoopLabelParser)
			...
			for i := 0; i < 100; i++ {
				...
				len := rand.Intn(chunks*entries-from) + 1
				iter, err := s.Iterator(context.TODO(), time.Unix(int64(from), 0), time.Unix(int64(from+len), 0), logproto.BACKWARD, nil, logql.NoopLabelParser)
				require.NotNil(t, iter)
				require.NoError(t, err)
