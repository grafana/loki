...
func TestMaxReturnedStreamsErrors(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			}, nil, recordPool.GetRecord(), 0, true, false, nil, "loki")
			require.NoError(t, err)
			...
			_, err = s.Push(context.Background(), newLines, nil, recordPool.GetRecord(), 0, true, false, nil, "loki")
			require.Error(t, err)
...
func TestPushRejectOldCounter(t *testing.T) {
	...
	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(1, 0), Line: "test"},
	}, nil, recordPool.GetRecord(), 2, true, false, nil, "loki")
	require.Equal(t, ErrEntriesExist, err)

	// succeed with a greater counter
	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(1, 0), Line: "test"},
	}, nil, recordPool.GetRecord(), 3, true, false, nil, "loki")
	require.Nil(t, err)
...
func TestUnorderedPush(t *testing.T) {
	...
	for _, x := range []struct {
		...
		written, err := s.Push(context.Background(), x.entries, nil, recordPool.GetRecord(), 0, true, false, nil, "loki")
		if x.err {
...
func TestPushRateLimit(t *testing.T) {
	...
	// Counter should be 2 now since the first line will be deduped.
	tracker := &mockUsageTracker{}
	_, err = s.Push(context.Background(), entries, nil, recordPool.GetRecord(), 0, true, true, tracker, "loki")
	require.Error(t, err)
...
func TestReplayAppendIgnoresValidityWindow(t *testing.T) {
	...
	_, err = s.Push(context.Background(), entries, nil, nil, 1, true, false, nil, "loki")
	require.Nil(t, err)
	...
	_, err = s.Push(context.Background(), entries, nil, recordPool.GetRecord(), 0, true, false, nil, "loki")
	require.NotNil(t, err)

	// Now pretend it's a replay. The same write should succeed.
	_, err = s.Push(context.Background(), entries, nil, nil, 2, true, false, nil, "loki")
	require.Nil(t, err)
