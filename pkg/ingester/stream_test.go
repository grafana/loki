...
func TestMaxReturnedStreamsErrors(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			}, false, recordPool.GetRecord(), 0, true, false, nil, "loki")
			require.NoError(t, err)
			...
			_, err = s.Push(context.Background(), newLines, false, recordPool.GetRecord(), 0, true, false, nil, "loki")
			require.Error(t, err)
...
func TestPushRejectOldCounter(t *testing.T) {
	...
	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(1, 0), Line: "test"},
	}, false, recordPool.GetRecord(), 2, true, false, nil, "loki")
	require.Equal(t, ErrEntriesExist, err)

	// succeed with a greater counter
	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(1, 0), Line: "test"},
	}, false, recordPool.GetRecord(), 3, true, false, nil, "loki")
	require.Nil(t, err)
...
func TestUnorderedPush(t *testing.T) {
	...
	for _, x := range []struct {
		...
		if x.cutBefore {
			_ = s.cutChunk(context.Background(), false)
		}
		written, err := s.Push(context.Background(), x.entries, false, recordPool.GetRecord(), 0, true, false, nil, "loki")
		if x.err {
...
func TestPushRateLimit(t *testing.T) {
	...
	// Counter should be 2 now since the first line will be deduped.
	tracker := &mockUsageTracker{}
	_, err = s.Push(context.Background(), entries, false, recordPool.GetRecord(), 0, true, true, tracker, "loki")
	require.Error(t, err)
...
func TestReplayAppendIgnoresValidityWindow(t *testing.T) {
	...
	_, err = s.Push(context.Background(), entries, false, nil, 1, true, false, nil, "loki")
	require.Nil(t, err)
	...
	_, err = s.Push(context.Background(), entries, false, recordPool.GetRecord(), 0, true, false, nil, "loki")
	require.NotNil(t, err)
...
func TestReplayPushStampsFirstSeenExactlyOnce(t *testing.T) {
	limits, err := validation.NewOverrides(defaultLimitsTestConfig(), nil)
	require.NoError(t, err)
	limiter := NewLimiter(limits, NilMetrics, newIngesterRingLimiterStrategy(&ringCountMock{count: 1}, 1), &TenantBasedStrategy{limits: limits})
	retentionHours := util.RetentionHours(limiter.limits.RetentionPeriod("fake"))
	...
	}, true, recordPool.GetRecord(), 0, true, false, nil, "loki")
	require.NoError(t, err)
	require.Len(t, s.chunks, 1)
	require.False(t, s.chunks[0].firstSeen.IsZero())

	firstSeen := s.chunks[0].firstSeen

	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(2, 0), Line: "second"},
	}, true, recordPool.GetRecord(), 0, true, false, nil, "loki")
	require.NoError(t, err)
	require.Equal(t, firstSeen, s.chunks[0].firstSeen)
}

func TestNonReplayPushLeavesFirstSeenZero(t *testing.T) {
	limits, err := validation.NewOverrides(defaultLimitsTestConfig(), nil)
	require.NoError(t, err)
	limiter := NewLimiter(limits, NilMetrics, newIngesterRingLimiterStrategy(&ringCountMock{count: 1}, 1), &TenantBasedStrategy{limits: limits})
	retentionHours := util.RetentionHours(limiter.limits.RetentionPeriod("fake"))
	...
	_, err = s.Push(context.Background(), []logproto.Entry{
		{Timestamp: time.Unix(1, 0), Line: "live"},
	}, false, recordPool.GetRecord(), 0, true, false, nil, "loki")
	require.NoError(t, err)
	require.Len(t, s.chunks, 1)
...
func Benchmark_PushStream(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		_, err := s.Push(ctx, e, false, rec, 0, true, false, nil, "loki")
		require.NoError(b, err)
