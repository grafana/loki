...
func Test_writeError(t *testing.T) {
	for _, tt := range []struct {
		...
		{"rpc cancelled",
			...
			"rpc error: code = Cancelled desc = context cancelled",
		...
		{"rpc cancelled multi",
			...
			"2 errors: rpc error: code = Cancelled desc = context cancelled; rpc error: code = Cancelled desc = context cancelled",
			http.StatusInternalServerError},
		{"mixed context and rpc cancelled",
			util.MultiError{context.Canceled, status.New(codes.Canceled, context.Canceled.Error()).Err()},
			"2 errors: context cancelled; rpc error: code = Cancelled desc = context cancelled",
			http.StatusInternalServerError},
		{"mixed context, rpc cancelled and another",
			...
			"3 errors: standard error; context cancelled; rpc error: code = Cancelled desc = context cancelled",
			http.StatusInternalServerError},
