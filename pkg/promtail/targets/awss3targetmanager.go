...
var (
	s3FailedTargets = promauto.NewCounterVec(prometheus.CounterOpts{
		Namespace: "promtail",
		Name:      "s3_targets_failed_total",
		Help:      "Number of failed targets.",
	}, []string{"reason"})
	s3TargetsActive = promauto.NewGauge(prometheus.GaugeOpts{
		Namespace: "promtail",
		Name:      "s3_targets_active_total",
...
type S3TargetManager struct {
	...
	syncers map[string]*s3TargetSyncer
...
func (tm *S3TargetManager) ActiveTargets() map[string][]Target {
...
func (tm *S3TargetManager) AllTargets() map[string][]Target {
	result := map[string][]Target{}
	for jobName, syncer := range tm.syncers {
		result[jobName] = append(result[jobName], syncer.ActiveTargets()...)
...
// s3TargetSyncer sync targets based on service discovery changes.
type s3TargetSyncer struct {
	log          log.Logger
	positions    positions.Positions
...
func (s *s3TargetSyncer) sync(groups []*targetgroup.Group) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *s3TargetSyncer) newTarget(path string, labels model.LabelSet, discoveredLabels model.LabelSet) (*FileTarget, error) {
	return NewFileTarget(s.log, s.entryHandler, s.positions, path, labels, discoveredLabels, s.targetConfig)
}

func (s *s3TargetSyncer) DroppedTargets() []Target {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	return append([]Target(nil), s.droppedTargets...)
}

func (s *s3TargetSyncer) ActiveTargets() []Target {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	actives := []Target{}
...
func (s *s3TargetSyncer) ready() bool {
	s.mtx.Lock()
	defer s.mtx.Unlock()
