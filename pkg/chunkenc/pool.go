...
var (
	...
	BytesBufferPool = pool.New(1<<9, 1<<13, 2, func(size int) any { return make([]byte, 0, size) })
	...
	LabelsPool = pool.New(1<<3, 1<<8, 2, func(size int) any { return make([][]byte, 0, size) })

	SymbolsPool = pool.New(1<<3, 1<<8, 2, func(size int) any { return make([]symbol, 0, size) })

	// SamplesPool pooling array of samples [512,1024,...,16k]
	SamplesPool = pool.New(1<<9, 1<<14, 2, func(size int) any { return make([]logproto.Sample, 0, size) })
