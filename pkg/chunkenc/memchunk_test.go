...
func TestBlock(t *testing.T) {
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			it, err := chk.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
			...
			sampleIt := chk.SampleIterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), nil, nil, logql.ExtractCount, logql.NoopLabelParser)
			idx = 0
			for sampleIt.Next() {
				s := sampleIt.Sample()
			...
			t.Run("bounded-iteration", func(t *testing.T) {
				it, err := chk.Iterator(context.Background(), time.Unix(0, 3), time.Unix(0, 7), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
				require.NoError(t, err)
...
func TestReadFormatV1(t *testing.T) {
	...
	it, err := r.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			assertLines := func(c *MemChunk) {
				...
				it, err := c.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
				if err != nil {
					t.Fatal(err)
				}
...
func TestSerialization(t *testing.T) {
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			it, err := bc.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
			require.NoError(t, err)
			for i := 0; i < numSamples; i++ {
			...
			sampleIt := bc.SampleIterator(context.Background(), time.Unix(0, 0), time.Unix(0, math.MaxInt64), nil, nil, logql.ExtractCount, logql.NoopLabelParser)
			for i := 0; i < numSamples; i++ {
...
func TestChunkFilling(t *testing.T) {
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			it, err := chk.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, 100), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
			require.NoError(t, err)
			i = 0
			for it.Next() {
				entry := it.Entry()
...
func TestChunkStats(t *testing.T) {
	...
	it, err := c.Iterator(ctx, first.Add(-time.Hour), entry.Timestamp.Add(time.Hour), logproto.BACKWARD, nil, logql.LineFilterFunc(func(line []byte) bool { return false }), logql.NoopLabelParser)
	if err != nil {
		t.Fatal(err)
	}
	for it.Next() {
	...
	ctx = stats.NewContext(context.Background())
	it, err = cb.Iterator(ctx, first.Add(-time.Hour), entry.Timestamp.Add(time.Hour), logproto.BACKWARD, nil, logql.LineFilterFunc(func(line []byte) bool { return false }), logql.NoopLabelParser)
	if err != nil {
		t.Fatal(err)
	}
	for it.Next() {
	...
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			for _, test := range []func(iter iter.EntryIterator, t *testing.T){
				...
				c := NewMemChunk(enc, testBlockSize, testTargetSize)
				inserted := fillChunk(c)
				iter, err := c.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, inserted), logproto.BACKWARD, nil, nil, logql.NoopLabelParser)
				if err != nil {
					t.Fatal(err)
				}
				test(iter, t)
		...
		b.Run(enc.String(), func(b *testing.B) {
			...
			for n := 0; n < b.N; n++ {
				for _, c := range chunks {
					...
					iterator, err := c.Iterator(context.Background(), time.Unix(0, 0), time.Now(), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
					if err != nil {
						panic(err)
					}
					for iterator.Next() {
...
func BenchmarkBackwardIterator(b *testing.B) {
	...
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		iterator, err := c.Iterator(context.Background(), time.Unix(0, 0), time.Now(), logproto.BACKWARD, nil, nil, logql.NoopLabelParser)
		if err != nil {
			panic(err)
		}
		for iterator.Next() {
...
func TestGenerateDataSize(t *testing.T) {
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			for _, c := range chunks {
				// use forward iterator for benchmark -- backward iterator does extra allocations by keeping entries in memory
				iterator, err := c.Iterator(context.TODO(), time.Unix(0, 0), time.Now(), logproto.FORWARD, nil, logql.LineFilterFunc(func(line []byte) bool {
...
func BenchmarkHeadBlockIterator(b *testing.B) {
	...
	for _, j := range []int{100000, 50000, 15000, 10000} {
		b.Run(fmt.Sprintf("Size %d", j), func(b *testing.B) {
			...
			for n := 0; n < b.N; n++ {
				iter := h.iterator(context.Background(), logproto.BACKWARD, 0, math.MaxInt64, nil, nil, logql.NoopLabelParser)
	...
	for _, tt := range []struct {
		...
		t.Run(
			...
			func(t *testing.T) {
				...
				// testing headchunk
				it, err := c.Iterator(context.Background(), tt.mint, tt.maxt, tt.direction, nil, nil, logql.NoopLabelParser)
				require.NoError(t, err)
...
func TestMemchunkLongLine(t *testing.T) {
	for _, enc := range testEncoding {
		t.Run(enc.String(), func(t *testing.T) {
			...
			it, err := c.Iterator(context.Background(), time.Unix(0, 0), time.Unix(0, 100), logproto.FORWARD, nil, nil, logql.NoopLabelParser)
			require.NoError(t, err)
			for i := 1; i <= 10; i++ {
				require.True(t, it.Next())
			}
