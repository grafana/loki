...
func TestPrefetchFilter_OptInHeader_Present(t *testing.T) {
	...
	require.Equal(t, req.StartTs, gotStart, "should preserve query start")
	require.Equal(t, req.EndTs, gotEnd, "should preserve query end")
}
...
func TestPrefetchFilter_AttachesHintRangesWithoutMutatingRequest(t *testing.T) {
	now := time.Now().Truncate(time.Millisecond)
	hintStart := now.Add(-45 * time.Minute)
	hintEnd := now.Add(-30 * time.Minute)
	...
	require.Nil(t, req.HintRanges, "filter middleware must not mutate the incoming request")
}
...
func TestPrefetchFilter_PreMinDateHintSource_RecordsPassthrough(t *testing.T) {
	...
	require.True(t, reqStart.Equal(gotStart), "pre-min-date hint should pass through original start")
	require.True(t, reqEnd.Equal(gotEnd), "pre-min-date hint should pass through original end")
...
func TestPrefetchFilter_PreMinDateAndIndexedHintsShareOneRequest(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Millisecond)
	reqStart := now.Add(-2 * time.Hour)
	reqEnd := now.Add(-30 * time.Minute)
	minDate := now.Add(-90 * time.Minute)
	indexedStart := now.Add(-60 * time.Minute)
	indexedEnd := now.Add(-50 * time.Minute)
	hp := &mockHintProvider{
		hints: &hintprovider.Hints{
			TimeRanges: []hintprovider.HintTimeRange{
				{Start: time.Time{}, End: minDate, Source: hintprovider.HintSourcePreMinDate},
				{Start: indexedStart, End: indexedEnd},
			},
		},
	}
	...
	require.Equal(t, []logproto.HintTimeRange{
		{Start: reqStart, End: minDate},
		{Start: indexedStart, End: indexedEnd},
	}, got.HintRanges)
	require.Equal(t, 1.0, testutil.ToFloat64(metrics.passthroughSubRequests.WithLabelValues("pre_min_date")))
}
...
func TestPrefetchFilter_MultipleHintRanges(t *testing.T) {
	...
	_, err := handler.Do(testTenantContextWithLive(), req)
	require.NoError(t, err)
	require.Equal(t, 1, nextCalled, "raw hints must not amplify downstream tasks")
	...
	require.Equal(t, 5*time.Minute, impact.queryDuration, "queried duration must be the sum of raw hints")
...
func TestPrefetchFilter_HintRangesClippedToRequest(t *testing.T) {
...
func TestPrefetchFilter_IngesterWindowPassthrough(t *testing.T) {
	...
	// intervals in the ingester window but attach hints to covered intervals.
	now := time.Now().Truncate(time.Millisecond)
	...
	querier := queryrangebase.HandlerFunc(func(_ context.Context, req queryrangebase.Request) (queryrangebase.Response, error) {
		lokiReq := req.(*queryrange.LokiRequest)
		mu.Lock()
		gotReqs = append(gotReqs, subReqInfo{
			start: lokiReq.StartTs,
			end:   lokiReq.EndTs,
			hints: lokiReq.HintRanges,
		})
		mu.Unlock()
		return emptyStreamResponse(), nil
	})
	...
	require.Equal(t, now.Add(-6*time.Hour), gotReqs[0].start)
	require.Equal(t, now.Add(-3*time.Hour), gotReqs[0].end)
	require.Equal(t, []logproto.HintTimeRange{{Start: hintRange.Start, End: hintRange.End}}, gotReqs[0].hints)
	require.Equal(t, now.Add(-3*time.Hour), gotReqs[1].start)
	require.Equal(t, now, gotReqs[1].end)
	require.Nil(t, gotReqs[1].hints)
}

func TestPrefetchFilter_24hQueryWith3hIngesterWindow(t *testing.T) {
	// Past-24h query, 3h ingester window, full coverage.
	// Covered interval should carry the hint range.
	...
	require.Len(t, gotReqs, 2)
	require.Equal(t, now.Add(-24*time.Hour), gotReqs[0].start)
	require.Equal(t, now.Add(-3*time.Hour), gotReqs[0].end)
	require.Equal(t, []logproto.HintTimeRange{{Start: hintRange.Start, End: hintRange.End}}, gotReqs[0].hints)
	require.Equal(t, now.Add(-3*time.Hour), gotReqs[1].start)
	require.Equal(t, now, gotReqs[1].end)
...
func TestPrefetchFilter_ImpactCountersAcrossSkipNarrowPassthrough(t *testing.T) {
	...
	// 2) covered overlap => hints attached
...
func TestDryRun_ForceLiveHeader_OverridesToLivePath(t *testing.T) {
	...
	require.Equal(t, req.StartTs, gotStart, "force-live should preserve query start")
	require.Equal(t, req.EndTs, gotEnd, "force-live should preserve query end")
...
func TestTenantMode_Live_OverridesGlobalDryRun(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 1, hp.Calls())
	require.Equal(t, req.StartTs, gotStart, "tenant live mode should preserve query start")
	require.Equal(t, req.EndTs, gotEnd, "tenant live mode should preserve query end")
