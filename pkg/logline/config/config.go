// Package config holds the "logline" section of Loki's config file.
//
// It is a separate package from the components that read it so that pkg/loki
// can embed the section. pkg/logline/queryfrontend cannot hold it: that
// package imports pkg/loki to read the assembled Loki config, so pkg/loki
...
const (
	...
	// DefaultShardPlanningEnabled and DefaultShardPlanningMinReductionRatio
	// are exported because the middleware applies them to a zero-valued
	// ShardPlanningConfig it receives directly, without going through
	// Validate.
	DefaultShardPlanningEnabled           = true
	DefaultShardPlanningMinReductionRatio = 0.75

	// middlewareFlagPrefix keeps the query-narrowing flags inside the logline
	// namespace instead of Loki's "query-frontend" one.
	middlewareFlagPrefix = "logline-query-frontend"
)
...
// ApplyDefaults fills zero-valued fields. It is exported because the
// middleware accepts a MiddlewareConfig built in code, which never passes
// through Validate.
...
type MiddlewareConfig struct {
	...
	// so it is hidden from the generated config reference: it has no flag and
	// no default of its own to document.
	QueryIngestersWithin time.Duration `yaml:"query_ingesters_within" doc:"hidden"`
...
//
// There is no unprefixed RegisterFlags: the natural prefix for these flags is
// "query-frontend", which is Loki's own flag namespace, so the prefix is
...
type Config struct {
	// Enabled turns on logline query narrowing in the query frontend.
	Enabled bool `yaml:"enabled"`
	// Store addresses the logline index in object storage. It is shared by
	// every logline component, so it sits at the top of the section rather
	// than under any one of them.
	Store         store.Config     `yaml:"store"`
	QueryFrontend MiddlewareConfig `yaml:"query_frontend"`
}

// RegisterFlags registers the flags for the logline section.
func (c *Config) RegisterFlags(f *flag.FlagSet) {
	...
	c.QueryFrontend.RegisterFlagsWithPrefix(middlewareFlagPrefix, f)
}

// Validate checks constraints and applies defaults.
//
// Store.Validate requires min_date, which has no sensible default, so this
// only runs when logline is enabled.
