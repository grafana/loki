...
// TestCaptureKafkaRecords connects to a port-forwarded WarpStream broker and saves
// raw Kafka records from partition 0 to disk for benchmarking.
//
// By default captures 2GB. Override with CAPTURE_MAX_MB.
//
// Run with:
//
//	CAPTURE_KAFKA=1 go test ./target/builder/ -run TestCaptureKafkaRecords -v -timeout 30m
//
// Requires: kubectl port-forward -n loki-dev-005 svc/warpstream-agent-read 9092:9092
func TestCaptureKafkaRecords(t *testing.T) {
	if os.Getenv("CAPTURE_KAFKA") == "" {
		t.Skip("Set CAPTURE_KAFKA=1 to capture records from a port-forwarded WarpStream broker")
	}

	broker := envOr("KAFKA_BROKER", "localhost:9092")
	topic := envOr("KAFKA_TOPIC", "ingest.dataobjs")
	saslUser := envOr("KAFKA_SASL_USER", "ccun_loki_dev_005")
	saslPass := envOr("KAFKA_SASL_PASS", "ccp_loki_dev_005")
	...
	opts := []kgo.Opt{
		...
		// Force all connections through the port-forward. WarpStream metadata
		// returns internal k8s IPs which aren't reachable from outside the cluster.
		kgo.Dialer(func(_ context.Context, _, _ string) (net.Conn, error) {
...
//
//	go test ./target/builder/ -bench BenchmarkCapturedPipeline -benchmem -benchtime 1x -timeout 30m
...
// Capture data first:
//
//	kubectl port-forward -n loki-dev-005 svc/warpstream-agent-read 9092:9092
//	for p in 0 1 2; do
//	  CAPTURE_KAFKA=1 KAFKA_PARTITION=$p CAPTURE_MAX_MB=200 go test ./target/builder/ -run TestCaptureKafkaRecords -v -timeout 10m
//	done
//
// Then run:
//
//	go test ./target/builder/ -bench BenchmarkPartitionComparison -benchmem -benchtime 1x -timeout 30m
...
//
//	BENCH_E2E=1 BENCH_DATA=testdata/kafka_records_p0.bin go test ./target/builder/ -run TestBuilderE2E -count=1 -v -timeout 60m
