...
func TestFilterNgramsForShard_Unsharded(t *testing.T) {
	...
	idx := logproto.HintIndex{ShardCount: 0}
	got := filterNgramsForShard(ngrams, idx)
...
func TestFilterNgramsForShard_MatchingShard(t *testing.T) {
	...
	ngrams := []string{"AAAAAA", "BBBBBB"}
	idx := logproto.HintIndex{ShardCount: 4, ShardAlgorithm: "first_byte", ShardValue: 1}
	got := filterNgramsForShard(ngrams, idx)
...
func TestFilterNgramsForShard_NoMatch(t *testing.T) {
	...
	ngrams := []string{"AAAAAA", "BBBBBB"}
	idx := logproto.HintIndex{ShardCount: 4, ShardAlgorithm: "first_byte", ShardValue: 3}
	got := filterNgramsForShard(ngrams, idx)
	require.Empty(t, got)
}
...
func TestFilterNgramsForShard_UnknownAlgorithm(t *testing.T) {
	...
	idx := logproto.HintIndex{ShardCount: 4, ShardAlgorithm: "future_v99", ShardValue: 0}
	got := filterNgramsForShard(ngrams, idx)
