...
func (m *SectionRef) unmarshal(dAtA []byte, depth int) error {
	...
	if l >= 256 && depth >= 0 {
		...
		if c := field3count; c > 0 {
			...
			if len(m.MinKey) == 0 && cap(m.MinKey) < c {
		...
		if c := field4count; c > 0 {
			...
			if len(m.MaxKey) == 0 && cap(m.MaxKey) < c {
...
func (m *SectionRef) UnmarshalNoPrescan(dAtA []byte) error {
...
func (m *RunRef) UnmarshalNoPrescan(dAtA []byte) error {
	return m.unmarshal(dAtA, -1)
}
...
func (m *TaskSpec) UnmarshalNoPrescan(dAtA []byte) error {
	return m.unmarshal(dAtA, -1)
}
