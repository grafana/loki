...
// run is the concrete [Run] built by calculateRuns.
//
...
// calculateRuns sorts sections in place and returns the [Run]s in creation
// order. Panics on a nil entry.
func calculateRuns(sections []*compactionv2pb.SectionRef) []*run {
	...
	// Iterate sorted sections and append each to the existing run whose
	// top_max_key is the largest value <= the section's MinKey (best-fit);
	// touching bounds (equal) keep the section in that run. Start a new run when
	// none is eligible. Among runs tied on top_max_key, the OLDEST run (first
	// seen) wins. Below is a demonstration of how this algorithm allows us to
	// generate runs and make compaction decisions.
	//
	...
	// largest top_max_key less than or equal to the section's MinKey (best-fit).
