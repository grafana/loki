...
// TestWindowedIterator_DisjointSectionsStayBounded proves the core property of
// the windowed merge: with N sections covering disjoint, ascending key ranges,
// only one is ever open at a time (each drains and closes before the next
// opens), even though all N are handed to the merge. Output stays sorted+complete.
func TestWindowedIterator_DisjointSectionsStayBounded(t *testing.T) {
	ctx := context.Background()
	sortSchema := []string{"label:app"}
	t0 := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)

	ent := func(n int) []push.Entry {
		out := make([]push.Entry, 0, n)
		for i := range n {
			out = append(out, push.Entry{Timestamp: t0.Add(time.Duration(i) * time.Second).UTC(), Line: "x"})
		}
		return out
	}

	appPairs := [][2]string{{"a", "b"}, {"c", "d"}, {"e", "f"}, {"g", "h"}, {"i", "j"}, {"k", "l"}}
	var objs []*dataobj.Object
	minKeys := make([]string, 0, len(appPairs))
	for _, p := range appPairs {
		obj, closer := buildSchemaObject(t, sortSchema, map[string][]push.Entry{
			`{app="` + p[0] + `"}`: ent(2),
			`{app="` + p[1] + `"}`: ent(2),
		})
		defer closer()
		objs = append(objs, obj)
		minKeys = append(minKeys, p[0]) // the section's smallest sort key
	}

	sections, remaps, globalSortKeys := planGlobalStreams(ctx, t, objs, sortSchema)
	require.Len(t, sections, len(objs), "each object has exactly one logs section")

	ws := make([]sortmerge.WindowSection, len(sections))
	for i := range sections {
		ws[i] = sortmerge.WindowSection{Section: sections[i], Remap: remaps[i], MinKey: minKeys[i]}
	}

	// The merge runs synchronously in this goroutine, so plain ints are safe.
	open, peak := 0, 0
	sortmerge.SetSectionHooksForTest(
		func() {
			open++
			if open > peak {
				peak = open
	...
	defer sortmerge.SetSectionHooksForTest(nil, nil)

	iter, err := sortmerge.WindowedIterator(ctx, ws, globalSortKeys, sortSchema)
	require.NoError(t, err)
	...
	for res := range iter {
		...
		require.NoError(t, err)
		if !first {
			require.LessOrEqual(t, recOrder(prev, rec), 0, "windowed output must be globally sorted")
		}
		prev, first = rec, false
		count++
	}

	require.Equal(t, len(appPairs)*2*2, count, "all rows present: N objs x 2 streams x 2 entries")
	require.Equal(t, 0, open, "every opened section reader must be closed")
	require.Equal(t, 1, peak, "disjoint sections must never be open concurrently")
}

// TestWindowedIterator_OverlappingSectionsMergeCorrectly verifies correctness when
// section key ranges overlap: a wide section spanning the whole range stays open
// across the narrow ones, and the output is still globally sorted and complete.
func TestWindowedIterator_OverlappingSectionsMergeCorrectly(t *testing.T) {
	ctx := context.Background()
	sortSchema := []string{"label:app"}
	t0 := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	...
	defs := []struct {
		...
		{[]string{"e", "f"}, "e"},
		{[]string{"g", "h"}, "g"},
		{[]string{"a", "h"}, "a"}, // wide: overlaps every narrow section
	}

	var objs []*dataobj.Object
	minKeys := make([]string, 0, len(defs))
	for i, d := range defs {
		byLabel := make(map[string][]push.Entry, len(d.apps))
		for _, a := range d.apps {
			// Offset the wide object's timestamps so records are distinct.
			byLabel[`{app="`+a+`"}`] = ent(t0.Add(time.Duration(i)*time.Hour), 2)
		}
		obj, closer := buildSchemaObject(t, sortSchema, byLabel)
		defer closer()
		objs = append(objs, obj)
		minKeys = append(minKeys, d.minKey)
	}

	sections, remaps, globalSortKeys := planGlobalStreams(ctx, t, objs, sortSchema)
	require.Len(t, sections, len(objs))
	...
	require.Equal(t, 5*2*2, count, "all rows present: 5 objs x 2 streams x 2 entries")
	// "a" and "h" each appear in a narrow section and in the wide section.
	require.Equal(t, 4, byApp["a"])
	require.Equal(t, 4, byApp["h"])
	require.Equal(t, 2, byApp["c"])
}
