...
func TestColumnValuesCalculation_TimestampsAndSizes(t *testing.T) {
	...
	// Timestamps: min=ts1 (10s), max=ts2 (30s).
...
func TestColumnValuesCalculation_EmptyBatch(t *testing.T) {
	...
	// No records → no postings (bloom filter would be empty but bloom postings are still appended
	// since the column was registered during Prepare). We just need Flush to not error.
	// The postings builder may or may not be created depending on whether any data flowed through.
	pb := builder.PostingsBuilderForTenant("tenant-1")
	// An empty bloom posting for trace_id will still be appended since the column was prepared.
	// Just verify no panic or error occurred.
	_ = pb
}
