...
func (c *statsCalculation) ProcessBatch(_ context.Context, ctx *logsCalculationContext, batch []logs.Record) error {
	for _, log := range batch {
		...
		if agg.rowCount == 0 || log.Timestamp.Before(agg.minTimestamp) {
			agg.minTimestamp = log.Timestamp
		}
		if agg.rowCount == 0 || log.Timestamp.After(agg.maxTimestamp) {
...
func (c *statsCalculation) Flush(_ context.Context, ctx *logsCalculationContext) error {
