...
func TestLabelPostingsCalculation_BitmapsNormalized(t *testing.T) {
	...
	// Stream 2 has the highest ID — all bitmaps should be normalized to include it.
	batch := []logs.Record{
	...
	// After normalization by the postings builder (which pads to equal lengths),
	// all bitmaps should have the same byte length.
	require.Equal(t, len(allPostings[0].StreamIDBitmap), len(allPostings[1].StreamIDBitmap),
...
func TestLabelPostingsCalculation_TimestampsAndSizes(t *testing.T) {
	...
	ts3 := time.Unix(20, 0).UTC()

	batch := []logs.Record{
		{StreamID: 1, Timestamp: ts1, Line: []byte("hello")},  // svcA, ts=10
		{StreamID: 1, Timestamp: ts2, Line: []byte("world!")}, // svcA, ts=30
		{StreamID: 1, Timestamp: ts3, Line: []byte("mid")},    // svcA, ts=20
	}
	require.NoError(t, calc.ProcessBatch(context.Background(), calcCtx, batch))
	require.NoError(t, calc.Flush(context.Background(), calcCtx))
	...
	require.Equal(t, ts2.UnixNano(), p.MaxTimestamp)
