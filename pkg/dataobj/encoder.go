...
// writer.
func newEncoder(store sectionScratchStore) *encoder {
	return &encoder{
...
func (enc *encoder) AppendSection(typ SectionType, data, metadata []byte) {
	handle := enc.store.Put(typ, data, metadata)
	enc.sectionHandles = append(enc.sectionHandles, handle)
...
func (enc *encoder) Metadata() (proto.Message, error) {
	sections := make([]*filemd.SectionInfo, len(enc.sectionHandles))

	offset := enc.startOffset

	for i, handle := range enc.sectionHandles {
		info, err := enc.store.Info(handle)
		if err != nil {
			return nil, err
		}
		...
		sections[i] = &filemd.SectionInfo{
			...
			Layout: &filemd.SectionLayout{
				...
				Metadata: &filemd.Region{
					Offset: uint64(offset + info.DataSize),
...
func (enc *encoder) Flush() (*snapshot, error) {
	...
	snapshot, err := newSnapshot(
		enc.store,
		magic, // header
		slices.Clone(enc.sectionHandles),
