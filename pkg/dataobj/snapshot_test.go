...
func Test_snapshot(t *testing.T) {
	store := newMemoryScratchStore()
	handles := []sectionHandle{
		store.Put(SectionType{}, []byte("example data 1"), []byte("example metadata 1")),
		store.Put(SectionType{}, []byte("example data 2"), []byte("example metadata 2")),
		store.Put(SectionType{}, []byte("example data 3"), []byte("example metadata 3")),
	}
	...
	snapshot, err := newSnapshot(store, header, handles, tailer)
	...
	t.Run("full section", func(t *testing.T) {
		info, err := store.Info(handles[0])
		require.NoError(t, err)

		fullSectionData := make([]byte, info.DataSize)
		n, err := snapshot.ReadAt(fullSectionData, int64(info.DataSize))
		require.NoError(t, err)
		require.Equal(t, info.DataSize, n)
	...
	for _, handle := range handles {
		...
		require.ErrorIs(t, err, errSectionHandleNotFound, "section should be deleted after closing a snapshot")
	}
}
