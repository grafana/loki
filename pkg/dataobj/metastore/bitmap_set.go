...
func unionBitmaps(a, b *memory.Bitmap) *memory.Bitmap {
	return combineBitmaps(compute.Or, a, b)
}
...
func intersectBitmaps(a, b *memory.Bitmap) *memory.Bitmap {
	return combineBitmaps(compute.And, a, b)
}
...
func differenceBitmaps(a, b *memory.Bitmap) *memory.Bitmap {
	...
	negated, err := compute.Not(nil, columnar.NewBool(right, memory.Bitmap{}), memory.Bitmap{})
	if err != nil {
		panic("metastore.differenceBitmaps: " + err.Error())
	}
	result, err := compute.And(nil, columnar.NewBool(left, memory.Bitmap{}), negated, memory.Bitmap{})
	if err != nil {
		panic("metastore.differenceBitmaps: " + err.Error())
	}
...
func combineBitmaps(op func(*memory.Allocator, columnar.Datum, columnar.Datum, memory.Bitmap) (columnar.Datum, error), a, b *memory.Bitmap) *memory.Bitmap {
	...
	if err != nil {
		panic("metastore.combineBitmaps: " + err.Error())
	}
	out := result.(*columnar.Bool).Values()
	return &out
}
