...
type scanPointers struct {
	region          *xcap.Region
	obj             *dataobj.Object
	streamPredicate streams.RowPredicate
...
func (s *scanPointers) findMatchingStreamIDs(ctx context.Context) ([]scalar.Scalar, error) {
	...
	err := forEachStream(ctx, s.obj, s.streamPredicate, func(stream streams.Stream) {
		matchingStreamIDs = append(matchingStreamIDs, scalar.NewInt64Scalar(stream.ID))
	})
	if err != nil {
