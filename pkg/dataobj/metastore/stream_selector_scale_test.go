...
// TestStreamSelector_OracleParity verifies the selector against an independent
// oracle on randomized single-section inputs. Uses deterministic iteration-based
// seeds for reproducibility.
func TestStreamSelector_OracleParity(t *testing.T) {
	...
	for seed := int64(0); seed < 300; seed++ {
		t.Run(fmt.Sprintf("seed_%d", seed), func(t *testing.T) {
			...
			secs, closer := buildLabelBloomSection(t, labelPostings, nil)
			defer closer()

			matchers := randomMatchers(rng, nMatchers)

			r := newStreamSelector(matchers, nil, time.Unix(0, 0), time.Unix(0, 2000000))
			got, err := r.selectStreams(ctx, secs)
			require.NoError(t, err)

			// Collect stream IDs from the selector result.
...
// TestStreamSelector_OracleParityMultiSection verifies the selector against
...
func TestStreamSelector_OracleParityMultiSection(t *testing.T) {
	ctx := context.Background()
	...
	for seed := int64(0); seed < 200; seed++ {
		t.Run(fmt.Sprintf("seed_%d", seed), func(t *testing.T) {
			...
			secs, closer := buildLabelBloomSection(t, allLabelPostings, nil)
			defer closer()
			...
			for _, sr := range got {
				key := fmt.Sprintf("%s/%d", sr.Section.ObjectPath, sr.Section.SectionIndex)
				if gotByKey[key] == nil {
...
// TestStreamSelector_EmptyCapableNameAbsentFromSection guards that an
...
func TestStreamSelector_EmptyCapableNameAbsentFromSection(t *testing.T) {
	ctx := context.Background()
...
// stream in the section, when the section survives. The selector records a name
...
// TestStreamSelector_OracleParityWithPredicates verifies the selector against
...
func TestStreamSelector_OracleParityWithPredicates(t *testing.T) {
	ctx := context.Background()
	...
	for seed := int64(0); seed < 300; seed++ {
		t.Run(fmt.Sprintf("seed_%d", seed), func(t *testing.T) {
			...
			secs, closer := buildLabelBloomSection(t, labelPostings, blooms)
			defer closer()

			r := newStreamSelector(matchers, preds, time.Unix(0, 0), time.Unix(0, 2000000))
			got, err := r.selectStreams(ctx, secs)
			require.NoError(t, err)
