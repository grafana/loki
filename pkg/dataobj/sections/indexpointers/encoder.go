...
const (
	indexPointersFormatVersion = 0x1
)
...
// encoder encodes an individual indexpointers section in a data object.
//
...
type encoder struct {
	...
	columns   []*indexpointersmd.ColumnDesc // closed columns.
	curColumn *indexpointersmd.ColumnDesc   // curColumn is the currently open column.
	sortInfo  *datasetmd.SectionSortInfo    // sort info for the indexpointers section.
}

// OpenColumn opens a new column in the indexpointers section. OpenColumn fails if
// there is another open column.
func (enc *encoder) OpenColumn(columnType indexpointersmd.ColumnType, info *dataset.ColumnInfo) (*columnEncoder, error) {
	...
	enc.curColumn = &indexpointersmd.ColumnDesc{
		Type: columnType,
...
// indexpointer. MetadataSize includes an estimate for the currently open element.
...
func (enc *encoder) Flush(w dataobj.SectionWriter) (int64, error) {
	...
	if err := streamio.WriteUvarint(metadataBuffer, indexPointersFormatVersion); err != nil {
		return 0, err
...
// columnEncoder encodes an individual column in a indexpointers section.
...
type columnEncoder struct {
	...
	pageHeaders []*indexpointersmd.PageDesc
...
func (enc *columnEncoder) AppendPage(page *dataset.MemPage) error {
	...
	// check it.
	enc.pageHeaders = append(enc.pageHeaders, &indexpointersmd.PageDesc{
...
func (enc *columnEncoder) Metadata() proto.Message {
	return &indexpointersmd.ColumnMetadata{Pages: enc.pageHeaders}
}
