...
func (rd *decoder) Columns(ctx context.Context) ([]*indexpointersmd.ColumnDesc, error) {
...
func (rd *decoder) Pages(ctx context.Context, columns []*indexpointersmd.ColumnDesc) result.Seq[[]*indexpointersmd.PageDesc] {
	return result.Iter(func(yield func([]*indexpointersmd.PageDesc) bool) error {
		results := make([][]*indexpointersmd.PageDesc, len(columns))

		columnInfo := func(c *indexpointersmd.ColumnDesc) (uint64, uint64) {
...
func (rd *decoder) ReadPages(ctx context.Context, pages []*indexpointersmd.PageDesc) result.Seq[dataset.PageData] {
	return result.Iter(func(yield func(dataset.PageData) bool) error {
		results := make([]dataset.PageData, len(pages))

		pageInfo := func(p *indexpointersmd.PageDesc) (uint64, uint64) {
...
func decodeStreamsMetadata(r streamio.Reader) (*indexpointersmd.Metadata, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("unexpected streams section format version: got=%d want=%d", gotVersion, indexPointersFormatVersion)
	}
...
func decodeStreamsColumnMetadata(r streamio.Reader) (*indexpointersmd.ColumnMetadata, error) {
