...
func (r *Reader) Read(ctx context.Context, batchSize int) (arrow.RecordBatch, error) {
	...
	// Build Arrow record from the rows
	builder := array.NewRecordBuilder(r.opts.Allocator, r.schema)

	// Process each row
	for _, row := range r.buf[:n] {
		// Process each column in the row
		for colIndex, val := range row.Values {
			// Skip columns not in projection list
			if colIndex >= len(r.opts.Columns) {
				continue
			}

			columnBuilder := builder.Field(colIndex)
			columnType := r.opts.Columns[colIndex].Type
			...
			case ColumnTypeInteger: // Appends IDs as int64
			...
			case ColumnTypeString: // Appends metadata and log lines as byte arrays
			...
			default:
				...
				// support reading it.
				return nil, fmt.Errorf("unsupported column type %s for column %d", columnType, colIndex)
			}
		}
	}
...
var columnDatatypes = map[ColumnType]arrow.DataType{
	...
	ColumnTypeInteger:   arrow.PrimitiveTypes.Int64,
	ColumnTypeTimestamp: arrow.FixedWidthTypes.Timestamp_ns,
	ColumnTypeString:    arrow.BinaryTypes.String,
}
