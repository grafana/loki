...
func Test_table_metadataCleanup(t *testing.T) {
	...
	_ = buf.Metadata("foo", pageSize, pageRows, dataset.NewZstdCompressionOptions())
	_ = buf.Metadata("bar", pageSize, pageRows, dataset.NewZstdCompressionOptions())
...
func initBuffer(buf *tableBuffer) {
	...
	buf.Message(pageSize, pageRows, dataset.NewZstdCompressionOptions())
}
...
func Test_mergeTables(t *testing.T) {
	...
	for _, strategy := range sortStrategies {
		t.Run(strategy.name, func(t *testing.T) {
			...
			tableA := buildTable(&buf, pageSize, pageRows, dataset.NewZstdCompressionOptions(), testRecords.tableA, strategy.sortOrder)
			tableB := buildTable(&buf, pageSize, pageRows, dataset.NewZstdCompressionOptions(), testRecords.tableB, strategy.sortOrder)
			tableC := buildTable(&buf, pageSize, pageRows, dataset.NewZstdCompressionOptions(), testRecords.tableC, strategy.sortOrder)

			// TableC should have been initially deduped by buildTable
			require.Equal(t, tableC.Timestamp.Desc.RowsCount, 2)

			mergedTable, err := mergeTables(&buf, pageSize, pageRows, dataset.NewZstdCompressionOptions(), []*table{tableA, tableB, tableC}, strategy.sortOrder)
			require.NoError(t, err)
...
func Test_table_backfillMetadata(t *testing.T) {
	...
	table := buildTable(&tableBuffer{}, pageSize, pageRows, dataset.NewZstdCompressionOptions(), records, SortTimestampDESC)
