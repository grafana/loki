...
func TestRankStreams_SameLabelsShareID(t *testing.T) {
	...
	leftID, err := ranks.Resolve(0, 2)
	require.NoError(t, err)
	rightID, err := ranks.Resolve(1, 5)
	require.NoError(t, err)
	...
	newLeft, resolvErr := ranks.Resolve(0, 7)
	require.NoError(t, resolvErr)
	require.NotEqual(t, leftID, newLeft)
	...
	for id := int64(2); id <= int64(count); id++ {
		...
		require.Negative(t, streams.CompareSortKey(prev, curr),
			"global stream IDs must increase in rank order")
	}
}
