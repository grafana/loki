...
func TestStreamRanks_SchemaRemap(t *testing.T) {
	ls := labels.FromStrings("app", "auth")
	src := map[int64]streams.Stream{
		3: {ID: 3, Labels: ls, ShardBucket: int64(streams.ShardBucket(ls))},
	}
	ranks, err := RankStreams([]string{"label:app"}, src)
	require.NoError(t, err)

	remap := schemaRemap(ranks, 0)
	require.Equal(t, int64(1), remap[3].newID)
	require.Equal(t, ranks.ByID(1).Key.streamSort(), remap[3].StreamSort)
}
