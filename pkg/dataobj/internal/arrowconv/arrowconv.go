...
// Arrow type.
//
// - [arrow.INT64] maps to [datasetmd.PhysicalType_PHYSICAL_TYPE_INT64].
// - [arrow.UINT64] maps to [datasetmd.PhysicalType_PHYSICAL_TYPE_UINT64].
// - [arrow.TIMESTAMP] maps to [datasetmd.PhysicalType_PHYSICAL_TYPE_INT64].
// - [arrow.STRING] maps to [datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY].
// - [arrow.BINARY] maps to [datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY].
//
// DatasetType returns [datasetmd.PhysicalType_PHYSICAL_TYPE_UNSPECIFIED], false for
// unsupported Arrow types.
func DatasetType(arrowType arrow.DataType) (datasetmd.PhysicalType, bool) {
	switch arrowType.ID() {
	case arrow.NULL:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_UNSPECIFIED, true
	case arrow.INT64:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_INT64, true
	case arrow.UINT64:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_UINT64, true
	case arrow.TIMESTAMP:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_INT64, true
	case arrow.STRING:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY, true
	case arrow.BINARY:
		return datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY, true
	}

	return datasetmd.PhysicalType_PHYSICAL_TYPE_UNSPECIFIED, false
}
...
//
// - For [datasetmd.PhysicalType_PHYSICAL_TYPE_INT64], s must be a [scalar.Int64] or [scalar.Timestamp].
// - For [datasetmd.PhysicalType_PHYSICAL_TYPE_UINT64], s must be a [scalar.Uint64].
// - For [datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY], s must be a [scalar.Binary] or [scalar.String].
//
...
//
//   - For [arrow.INT64], v must be a [datasetmd.PhysicalType_PHYSICAL_TYPE_INT64].
//   - For [arrow.UINT64], v must be a [datasetmd.PhysicalType_PHYSICAL_TYPE_UINT64].
//   - For [arrow.TIMESTAMP], v must be a [datasetmd.PhysicalType_PHYSICAL_TYPE_INT64], which
//     will be converted into a nanosecond timestamp.
//   - For [arrow.STRING], v must be a [datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY].
//   - For [arrow.BINARY], v must be a [datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY].
//
...
func ToScalar(v dataset.Value, toType arrow.DataType) scalar.Scalar {
	...
	case arrow.INT64:
		if got, want := v.Type(), datasetmd.PhysicalType_PHYSICAL_TYPE_INT64; got != want {
	...
	case arrow.UINT64:
		if got, want := v.Type(), datasetmd.PhysicalType_PHYSICAL_TYPE_UINT64; got != want {
	...
	case arrow.STRING:
		if got, want := v.Type(), datasetmd.PhysicalType_PHYSICAL_TYPE_BINARY; got != want {
