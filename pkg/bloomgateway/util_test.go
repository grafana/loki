...
func Test_SpansMultipleDays(t *testing.T) {
	testCases := map[string]struct {
		...
		"from and through are within same day": {
			inp: []*logproto.GroupedChunkRefs{
				{
					Fingerprint: 0x00,
					Refs: []*logproto.ShortRef{
						{From: mktime("2024-04-17 00:00"), Through: mktime("2024-04-17 11:59")},
					},
				},
				{
					Fingerprint: 0x01,
					Refs: []*logproto.ShortRef{
						{From: mktime("2024-04-17 12:00"), Through: mktime("2024-04-17 23:59")},
					},
				},
			},
			exp: false,
		},
		"from is within same day, through is on next day": {
			inp: []*logproto.GroupedChunkRefs{
				{
					Fingerprint: 0x00,
					Refs: []*logproto.ShortRef{
				...
				},
				{
					Fingerprint: 0x01,
					Refs: []*logproto.ShortRef{
						{From: mktime("2024-04-17 06:00"), Through: mktime("2024-04-17 17:59")},
						{From: mktime("2024-04-17 18:00"), Through: mktime("2024-04-18 05:59")},
					},
				},
			},
			exp: false,
		},
		"from times are on different days": {
			inp: []*logproto.GroupedChunkRefs{
				{
					Fingerprint: 0x00,
					Refs: []*logproto.ShortRef{
				...
				},
				{
					Fingerprint: 0x01,
					Refs: []*logproto.ShortRef{
						{From: mktime("2024-04-18 00:00"), Through: mktime("2024-04-18 11:59")},
					},
				},
			},
			exp: true,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			result := spansMultipleDays(&logproto.FilterChunkRefRequest{Refs: tc.inp})
			require.Equal(t, tc.exp, result)
		})
	}
}
