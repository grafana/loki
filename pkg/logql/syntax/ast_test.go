...
func Test_SampleExpr_String(t *testing.T) {
	...
	for _, tc := range []string{
		...
		`,
		`buckets_over_time(1.1, {namespace="tns"} | pattern "<_> - <request_time>" | unwrap request_time[5m]) by (path)`,
		`buckets_over_time(1.1, {namespace="tns"} | pattern "<_> - <request_time>" | unwrap request_time[5m])`,
	} {
