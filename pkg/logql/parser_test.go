	...
	for _, tc := range []struct {
		...
		},
		{
			...
			exp: &rangeAggregationExpr{
				...
				left: &logRange{
					left: newPipelineExpr(
						newMatcherExpr([]*labels.Matcher{{Type: labels.MatchEqual, Name: "foo", Value: "bar"}}),
						MultiPipelineExpr{newLineFilterExpr(nil, labels.MatchEqual, "error")},
					),
					interval: 12 * time.Hour,
				},
			},
		...
		},
		{
			...
			exp: newPipelineExpr(
				...
				MultiPipelineExpr{newLineFilterExpr(nil, labels.MatchEqual, "baz")},
			),
		},
		{
		...
		},
		{
			...
			exp: &pipelineExpr{
				...
				pipeline: MultiPipelineExpr{
					...
					&labelFilterExpr{
						Filterer: labelfilter.NewAnd(
							labelfilter.NewString(mustNewMatcher(labels.MatchEqual, "foo", "bar")),
							labelfilter.NewAnd(
								labelfilter.NewString(mustNewMatcher(labels.MatchNotEqual, "buzz", "blip")),
								labelfilter.NewOr(
									labelfilter.NewString(mustNewMatcher(labels.MatchRegexp, "blop", "boop")),
									labelfilter.NewNumeric(labelfilter.FilterEqual, "fuzz", 5),
								),
							),
						),
					},
