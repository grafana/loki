...
func Test_lineFormatter_Format(t *testing.T) {
	tests := []struct {
		...
		{
			"decryptAES valid",
			newMustLineFormatter(`{{ .foo | decryptAES "secretkey" }}-{{ decryptAES "secretkey" .foo }}`),
			labels.FromStrings("foo", "30tEfhuJSVRhpG97XCuWgz2okj7L8vQ1s6V9zVUPeDQ="),
			1656353124120000000,
		...
		{
			"decryptAES invalid base64",
			newMustLineFormatter(`{{ .foo | decryptAES "secretkey" }}`),
			labels.FromStrings("foo", `\`),
			1656353124120000000,
			[]byte("1"),
			labels.FromStrings("foo", `\`,
				"__error__", "TemplateFormatErr",
				"__error_details__", "template: line:1:10: executing \"line\" at <decryptAES \"secretkey\">: error calling decryptAES: illegal base64 data at input byte 0"),
			[]byte("1"),
		},
		{
			"decryptAES invalid ciphertext",
			newMustLineFormatter(`{{ .foo | decryptAES "secretkey" }}`),
			labels.FromStrings("foo", "b2ggbm8h"),
			1656353124120000000,
			[]byte("1"),
			labels.FromStrings("foo", `b2ggbm8h`,
				"__error__", "TemplateFormatErr",
				"__error_details__", "template: line:1:10: executing \"line\" at <decryptAES \"secretkey\">: error calling decryptAES: runtime error: slice bounds out of range [:16] with capacity 6"),
			[]byte("1"),
		},
		{
			"alignLeft",
...
func Test_labelsFormatter_Format(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"decryptAES valid",
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("bar", "{{ .foo | decryptAES \"secretkey\" }}")}),
			labels.FromStrings("foo", "30tEfhuJSVRhpG97XCuWgz2okj7L8vQ1s6V9zVUPeDQ="),
			labels.FromStrings("foo", "30tEfhuJSVRhpG97XCuWgz2okj7L8vQ1s6V9zVUPeDQ=",
				"bar", "plaintext",
			),
		...
		},
		{
			...
			labels.FromStrings("foo", `\`,
				"__error__", "TemplateFormatErr",
				"__error_details__", "template: label:1:10: executing \"label\" at <decryptAES \"secretkey\">: error calling decryptAES: illegal base64 data at input byte 0"),
		},
		{
			"decryptAES invalid ciphertext",
			mustNewLabelsFormatter([]LabelFmt{NewTemplateLabelFmt("bar", `{{ .foo | decryptAES "secretkey" }}`)}),
			...
			labels.FromStrings("foo", `b2ggbm8h`,
				"__error__", "TemplateFormatErr",
				"__error_details__", "template: label:1:10: executing \"label\" at <decryptAES \"secretkey\">: error calling decryptAES: runtime error: slice bounds out of range [:16] with capacity 6"),
		},
		{
			"unixToTime days",
