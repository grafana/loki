...
func Test_labelSampleExtractor_Extract(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			outval, outlbs, _, ok := tt.ex.ForStream(tt.in).Process(0, []byte(tt.line))
			require.Equal(t, tt.wantOk, ok)
			require.Equal(t, tt.want, outval)
			require.Equal(t, tt.wantLbs, outlbs.Labels())

			outval, outlbs, _, ok = tt.ex.ForStream(tt.in).ProcessString(0, tt.line)
			require.Equal(t, tt.wantOk, ok)
			require.Equal(t, tt.want, outval)
			require.Equal(t, tt.wantLbs, outlbs.Labels())
		})
...
func Test_Extract_ExpectedLabels(t *testing.T) {
	...
	f, lbs, _, ok := ex.ForStream(labels.Labels{{Name: "bar", Value: "foo"}}).ProcessString(0, `{"duration":"20ms","foo":"json"}`)
	require.True(t, ok)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			for _, line := range tc.checkLines {
				v, lbs, _, ok := tc.extractor.ForStream(labels.Labels{{Name: "bar", Value: "foo"}}).ProcessString(0, line.logLine)
				skipped := !ok
...
func TestNewLineSampleExtractor(t *testing.T) {
	...
	f, l, _, ok := sse.Process(0, []byte(`foo`))
	require.True(t, ok)
	require.Equal(t, 1., f)
	assertLabelResult(t, lbs, l)

	f, l, _, ok = sse.ProcessString(0, `foo`)
	require.True(t, ok)
	require.Equal(t, 1., f)
	assertLabelResult(t, lbs, l)
	...
	sse = se.ForStream(lbs)
	_, _, _, ok = sse.Process(0, []byte(`nope`))
	require.False(t, ok)
}
...
func TestFilteringSampleExtractor(t *testing.T) {
	...
	for _, test := range tt {
		t.Run(test.name, func(t *testing.T) {
			_, _, _, ok := se.ForStream(test.labels).Process(test.ts, []byte(test.line))
			require.Equal(t, test.ok, ok)

			_, _, _, ok = se.ForStream(test.labels).ProcessString(test.ts, test.line)
			require.Equal(t, test.ok, ok)
		})
	}
}
...
func (p *stubStreamExtractor) Process(ts int64, line []byte) (float64, LabelsResult, int64, bool) {
	return 0, nil, ts, true
}

func (p *stubStreamExtractor) ProcessString(ts int64, line string) (float64, LabelsResult, int64, bool) {
	return 0, nil, ts, true
}
