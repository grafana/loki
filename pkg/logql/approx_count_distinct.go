...
func (m CountDistinctMatrix) ToProto() (*logproto.CountDistinctMatrix, error) {
	values := make([]*logproto.CountDistinctVector, len(m))
	for i, vec := range m {
		p, err := vec.ToProto()
		if err != nil {
			return nil, err
	...
	return &logproto.CountDistinctMatrix{Values: values}, nil
}
...
// CountDistinctMatrixFromProto deserializes a CountDistinctMatrix.
func CountDistinctMatrixFromProto(proto *logproto.CountDistinctMatrix) (CountDistinctMatrix, error) {
	if proto == nil {
		return CountDistinctMatrix{}, nil
	}
	out := make(CountDistinctMatrix, len(proto.Values))
	for i, vec := range proto.Values {
		v, err := CountDistinctVectorFromProto(vec)
		if err != nil {
			return nil, err
		}
		out[i] = v
