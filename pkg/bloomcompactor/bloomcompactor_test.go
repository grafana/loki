	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, boundsA := range compactorOwnershipRange {
				...
				// TODO(salvacorts): Looks like the key-sapce is 0-MaxUint32, shouldn't it be 0-MaxUint64?
				fpPerTenant := math.MaxUint32 / uint64(tc.expectedCompactorsOwningTenant)
			...
			// TODO:
			//  x create #ringInstances compactors
			//  x wait for all rings to see each other
			//  x call ownsTenant with each compactor and store the result
			//  x assert that #shardSize compactors own the tenant
			//  ~ assert that the fingerprint key-space is evenly distributed across the compactors
			//  x assert that the fingerprint key-space is not overlapping
			//  ~ assert that the fingerprint key-space is complete
		})
	}
}
