...
func (w *binaryWriter) Append(arr columnar.Array) error {
	...
	// Append data bytes.
	w.data.Grow(len(srcData))
	w.data.Append(srcData...)
	...
	// first since the leading zero was written by init).
	{
		...
		for i := range utf8Arr.Len() {
			rebasedBuilder.AppendValue(srcOffsets[i+1] + baseOffset)
		}
...
func (w *binaryWriter) Flush(ctx context.Context, sink Sink) (Array, error) {
	defer w.reset()
...
func (r *binaryReader) Read(ctx context.Context, alloc *memory.Allocator, count int) (columnar.Array, error) {
	...
	endOff := min(r.off+count, r.arr.Stats.RowCount)
	n := endOff - r.off
	if n == 0 {
		return nil, io.EOF
	}
