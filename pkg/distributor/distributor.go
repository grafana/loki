...
func labelTemplate(lbls string) labels.Labels {
	...
	streamLabels[len(baseLbls)] = labels.Label{Name: log.ShardLbName, Value: log.ShardLbPlaceholder}
...
func (d *Distributor) createShard(stream logproto.Stream, lbls labels.Labels, streamPattern string, totalShards, shardNumber int) (logproto.Stream, bool) {
	...
	lbls[len(lbls)-1] = labels.Label{Name: log.ShardLbName, Value: shardLabel}
	return logproto.Stream{
		Labels:  strings.Replace(streamPattern, log.ShardLbPlaceholder, shardLabel, 1),
		Hash:    lbls.Hash(),
