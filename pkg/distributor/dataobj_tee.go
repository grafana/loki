...
func (c *DataObjTeeConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.DurationVar(&c.RateBatchWindow, "distributor.dataobj-tee.rate-batch-window", 30*time.Second, "Duration to accumulate rate updates before sending to limits frontend. Set to 0 to disable batching.")
}
