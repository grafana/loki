...
func Benchmark_SortLabelsOnPush(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		_, _, _, _, _, err := d.parseStreamLabels(vCtx, stream.Labels, stream, streamResolver, "loki")
		if err != nil {
...
func TestDistributor_StructuredMetadataSanitization(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		require.NoError(t, err)
		assert.Equal(t, tc.expectedResponse, response)
		assert.Equal(t, tc.numSanitizations, testutil.ToFloat64(distributors[0].tenantPushSanitizedStructuredMetadata.WithLabelValues("test", "loki")))
	}
}
