...
func SingleFlightHandler(name string, gc *singleflight.SingleFlight, log log.Logger, next http.RoundTripper, codec queryrangebase.Codec) queryrangebase.Handler {
	singleFlight := gc.NewGroup(name, makeRequest(next, codec, log, name))
...
func makeRequest(next http.RoundTripper, codec queryrangebase.Codec, log log.Logger, name string) groupcache.GetterFunc {
