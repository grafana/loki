...
func TestNewProjectionPlan(t *testing.T) {
	tests := map[string]struct {
		...
		wantMetadata   []string
		wantPredicates []logs.RowPredicate
		// wantSectionPredicates lists the matchers handed to the metastore, as strings.
		...
		"an equality against an empty value is pushed to the reader but withheld from the metastore": {
			...
			wantPredicates: []logs.RowPredicate{logs.MetadataMatcherRowPredicate{Key: "level", Value: ""}},
		},
		...
		"a metadata inequality is pushed as a filter predicate": {
			...
			wantColumns:           []logs.ColumnType{logs.ColumnTypeStreamID, logs.ColumnTypeTimestamp},
			wantMetadata:          []string{"app", "level"},
			wantPredicates:        []logs.RowPredicate{logs.MetadataFilterRowPredicate{Key: "level"}},
			wantSectionPredicates: []string{`level!="error"`},
		},
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			requirePredicatesEqual(t, test.wantPredicates, predicates)
