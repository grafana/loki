...
func TestQuerier_Label_QueryTimeoutConfigFlag(t *testing.T) {
	...
	store := newStoreMock()
	store.On("LabelValuesForMetricName", mock.Anything, mock.Anything, model.TimeFromUnixNano(startTime.UnixNano()), model.TimeFromUnixNano(endTime.UnixNano()), "logs", "test").Return([]string{"foo", "bar"}, nil)
