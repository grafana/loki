...
func TestDataObjLogReader_MultiObject(t *testing.T) {
	...
	tasks := newDataObjReadPlanner(metastoreSectionsResolver{ms: ms}, cache, false).plan(ctx, at(0), at(100), matchers, nil, expr)
...
func TestDataObjLogReader_ConcurrentSections(t *testing.T) {
	...
	tasks := drainTaskIterator(t, newDataObjReadPlanner(metastoreSectionsResolver{ms: ms}, cache, false).plan(ctx, at(0), at(100), matchers, nil, expr))
