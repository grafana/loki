	...
	for !t.stopped {
		...
		case <-checkConnectionTicker.C:
			...
			if err := t.checkIngesterConnections(); err != nil {
				level.Error(logutil.Logger).Log("msg", "Error reconnecting to disconnected ingesters", "err", err)
			}
		case <-tailMaxDurationTicker.C:
			if err := t.close(); err != nil {
				level.Error(logutil.Logger).Log("msg", "Error closing Tailer", "err", err)
			}
		...
		if len(tailResponse.Streams) == 0 {
			...
			if numClients == 0 {
				...
				if err := t.checkIngesterConnections(); err != nil {
					level.Error(logutil.Logger).Log("msg", "Error reconnecting to ingesters", "err", err)
				} else {
					continue
				}
				if err := t.close(); err != nil {
...
func (t *Tailer) readTailClient(addr string, querierTailClient logproto.Querier_TailClient) {
	...
	logger := logutil.WithContext(querierTailClient.Context(), logutil.Logger)
	for {
		if t.stopped {
