...
func (cfg *Config) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&cfg.MaxConcurrent, "querier.max-concurrent", 20, `The maximum number of top-level LogQL queries that will execute at the same time, per querier process. It should be set to match the number of CPU cores available on the querier (or slightly less than the number of cores if you are in a shared environment). If using the query frontend, Then you should run enough queriers to match ('-querier.worker-parallelism' * 'number of query frontend replicas')/'-querier.max-concurrent'. Otherwise queries may queue in the query frontend, which will affect QoS. Alternatively, consider using '-querier.worker-match-max-concurrent' to force worker parallelism to match '-querier.max-concurrent'`)
