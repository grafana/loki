	...
	tests := []struct {
		...
		},
		{
			...
			patterns: []string{
				...
				`logger=ngalert.state.manager user=893151 slug=cmtdsnp instance="cluster=tds-np-cluster, container=<_> instance=<_> job=integrations/kubernetes/kube-state-metrics, namespace=tds, pod=<_> uid=<_> t=<_> level=debug msg="Setting next state" handler=resultNormal`,
				`logger=ngalert.state.manager user=893151 slug=cmtdsnp instance="cluster=tds-np-cluster, container=<_> instance=<_> job=integrations/kubernetes/kube-state-metrics, namespace=tds-devops, pod=exo-devca-cicd-288-zcl2b-9ws4z-nzgt7, uid=ca99b6a7-f08f-475a-adf6-dcf8c8936eed" t=<_> level=debug msg="Setting next state" handler=resultNormal`,
