...
func (m *Metastore) doMark() error {
	olderThan := time.Now().Add(-m.config.RetentionPeriod)
	req := &raftlogpb.MarkCommand{Timestamp: uint64(olderThan.UnixMilli())}
	_, _, err := applyCommand[*raftlogpb.MarkCommand, *anypb.Any](m.raft, req, m.config.Raft.ApplyTimeout)
	return err
}
...
func (m *Metastore) doSweep() error {
	olderThan := time.Now().Add(-m.config.RetentionPeriod - m.config.RetentionGracePeriod)
	...
	_, _, err := applyCommand[*raftlogpb.SweepCommand, *anypb.Any](m.raft, req, m.config.Raft.ApplyTimeout)
	return err
}
