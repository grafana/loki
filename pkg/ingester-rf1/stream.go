...
func fromWireChunks(conf *Config, headfmt chunkenc.HeadBlockFmt, wireChunks []ingester.Chunk) ([]chunkDesc, error) {
...
func (s *stream) setChunks(chunks []ingester.Chunk) (bytesAdded, entriesAdded int, err error) {
	s.chunkMtx.Lock()
...
func (s *stream) Push(
	...
	bytesAdded, _, entriesWithErr := s.storeEntries(ctx, toStore, usageTracker)
