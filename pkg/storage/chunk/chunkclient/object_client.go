...
// chunkClient wraps an ObjectClient to store and retrieve chunks in object store backends
...
func NewClient(store client.ObjectClient, encoder KeyEncoder, schema config.SchemaConfig) Client {
	return NewClientWithMaxParallel(store, encoder, defaultMaxParallel, schema)
}

func NewClientWithMaxParallel(store client.ObjectClient, encoder KeyEncoder, maxParallel int, schema config.SchemaConfig) Client {
	return &chunkClient{
		store:               store,
		keyEncoder:          encoder,
...
func (o *chunkClient) PutChunks(ctx context.Context, chunks []chunk.Chunk) error {
	var (
		chunkKeys []string
		chunkBufs [][]byte
	)
...
func (o *chunkClient) GetChunks(ctx context.Context, chunks []chunk.Chunk) ([]chunk.Chunk, error) {
...
func (o *chunkClient) getChunk(ctx context.Context, decodeContext *chunk.DecodeContext, c chunk.Chunk) (chunk.Chunk, error) {
	if ctx.Err() != nil {
...
// GetChunks retrieves the specified chunks from the configured backend
func (o *chunkClient) DeleteChunk(ctx context.Context, userID, chunkID string) error {
	key := chunkID
	if o.keyEncoder != nil {
...
func (o *chunkClient) IsChunkNotFoundErr(err error) bool {
	return o.store.IsObjectNotFoundErr(err)
}

func (o *chunkClient) IsRetryableErr(err error) bool {
