...
func TestMemcached_fetchKeysBatched(t *testing.T) {
	...
	t.Run("stop fetching in-between, when context cancelled", func(t *testing.T) {
		...
		m.SetFetchDelayForTest(delayCh) // make it wait in specific `select` case that does actual fetch.
		...
		close(wait)                                                                                                 // start the fetching
		go func() { time.Sleep(10 * time.Millisecond); close(delayCh); ctxCancel(); m.SetFetchDelayForTest(nil) }() // cancel after fetching begins
		wg.Wait()
