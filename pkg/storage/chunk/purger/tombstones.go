...
func (tl *TombstonesLoader) loop() {
	...
	for {
		...
		case <-tombstonesReloadTimer.C:
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error reloading tombstones", "err", err)
			}
		case <-tl.quit:
			return
		}
...
func (tl *TombstonesLoader) getCacheGenNumbersPerTenants(tenantIDs []string) *cacheGenNumbers {
	...
	for pos, tenantID := range tenantIDs {
		...
		if numbers.results != "" {
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error parsing resultsCacheGenNumber", "user", tenantID, "err", err)
		...
		if numbers.store != "" {
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error parsing storeCacheGenNumber", "user", tenantID, "err", err)
...
func (tl *TombstonesLoader) getCacheGenNumbers(userID string) *cacheGenNumbers {
	...
	if err != nil {
		level.Error(logutil.Logger).Log("msg", "error loading cache generation numbers", "err", err)
