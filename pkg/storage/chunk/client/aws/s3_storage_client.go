...
func (cfg *S3Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.StringVar(&cfg.StorageClass, prefix+"s3.storage-class", storageawscommon.StorageClassStandard, fmt.Sprintf("The S3 storage class which objects will use. Supported values are: %s. EXPRESS_ONEZONE is only accepted by S3 Express One Zone directory buckets, which in turn reject every other storage class.", strings.Join(storageawscommon.SupportedStorageClasses, ", ")))
