...
func New(cache cache.Cache, cachel2 cache.Cache, cacheStubs bool, schema config.SchemaConfig, storage chunkclient.Client, l2CacheHandoff time.Duration, skipQueryWritebackOlderThan time.Duration) (*Fetcher, error) {
	c := &Fetcher{
		schema:                           schema,
		storage:                          storage,
		cache:                            cache,
...
func (c *Fetcher) Client() chunkclient.Client {
	return c.storage
}
