...
func NewChunkClient(name string, cfg Config, schemaCfg chunk.SchemaConfig, registerer prometheus.Registerer) (chunk.Client, error) {
	...
	case StorageTypeAWS, StorageTypeS3:
		store, err := aws.NewS3ObjectClient(cfg.AWSStorageConfig.S3Config)
	...
	case StorageTypeAzure:
		store, err := azure.NewBlobStorage(&cfg.AzureStorageConfig)
		return newChunkClientFromStore(store, err, schemaCfg)
	case StorageTypeGCP:
	...
	case StorageTypeGCPColumnKey, StorageTypeBigTable, StorageTypeBigTableHashed:
		return gcp.NewBigtableObjectClient(context.Background(), cfg.GCPStorageConfig, schemaCfg)
	case StorageTypeGCS:
		store, err := gcp.NewGCSObjectClient(context.Background(), cfg.GCSConfig)
		return newChunkClientFromStore(store, err, schemaCfg)
	case StorageTypeSwift:
		store, err := openstack.NewSwiftObjectClient(cfg.Swift)
		return newChunkClientFromStore(store, err, schemaCfg)
	case StorageTypeCassandra:
