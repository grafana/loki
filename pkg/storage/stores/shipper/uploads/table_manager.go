...
func (tm *TableManager) Stop() {
	_ = level.Info(util_log.Logger).Log("msg", "stopping table manager")
...
func (tm *TableManager) uploadTables(ctx context.Context, force bool) {
	...
	_ = level.Info(util_log.Logger).Log("msg", "uploading tables")
	...
	for _, table := range tm.tables {
		...
		if err != nil {
			...
			_ = level.Error(util_log.Logger).Log("msg", "failed to snapshot table for reads", "table", table.name, "err", err)
		}
		...
		if err != nil {
			...
			_ = level.Error(util_log.Logger).Log("msg", "failed to upload dbs", "table", table.name, "err", err)
			continue
		}
		...
		if err != nil {
			...
			_ = level.Error(util_log.Logger).Log("msg", "failed to cleanup uploaded dbs past their retention period", "table", table.name, "err", err)
		}
	}
...
func (tm *TableManager) loadTables() (map[string]*Table, error) {
	...
	for _, fileInfo := range filesInfo {
		...
		if !fileInfo.IsDir() {
			_ = level.Info(util_log.Logger).Log("msg", fmt.Sprintf("found a legacy file %s, moving it to folder with same name", fileInfo.Name()))
		...
		_ = level.Info(util_log.Logger).Log("msg", fmt.Sprintf("loading table %s", fileInfo.Name()))
		...
		if table == nil {
			...
			if err != nil {
				_ = level.Error(util_log.Logger).Log("msg", "failed to remove empty table folder", "table", fileInfo.Name(), "err", err)
			}
			continue
		}
