...
func (tm *TableManager) Stop() {
	level.Info(logutil.Logger).Log("msg", "stopping table manager")
...
func (tm *TableManager) uploadTables(ctx context.Context, force bool) {
	...
	level.Info(logutil.Logger).Log("msg", "uploading tables")
	...
	for _, table := range tm.tables {
		...
		if err != nil {
			...
			level.Error(logutil.Logger).Log("msg", "failed to snapshot table for reads", "table", table.name, "err", err)
		}
		...
		if err != nil {
			...
			level.Error(logutil.Logger).Log("msg", "failed to upload dbs", "table", table.name, "err", err)
			continue
		}
		...
		if err != nil {
			...
			level.Error(logutil.Logger).Log("msg", "failed to cleanup uploaded dbs past their retention period", "table", table.name, "err", err)
		}
	}
...
func (tm *TableManager) loadTables() (map[string]*Table, error) {
	...
	for _, fileInfo := range filesInfo {
		...
		if !fileInfo.IsDir() {
			level.Info(logutil.Logger).Log("msg", fmt.Sprintf("found a legacy file %s, moving it to folder with same name", fileInfo.Name()))
		...
		level.Info(logutil.Logger).Log("msg", fmt.Sprintf("loading table %s", fileInfo.Name()))
		...
		if table == nil {
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "failed to remove empty table folder", "table", fileInfo.Name(), "err", err)
			}
			continue
		}
