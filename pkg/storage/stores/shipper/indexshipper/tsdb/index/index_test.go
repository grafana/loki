...
func TestFormatV4_StreamID_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	fn := filepath.Join(dir, IndexFilename)
	...
	series := labels.FromStrings("app", "gateway", "cluster", "us")
	require.NoError(t, iw.AddSymbol("app"))
	require.NoError(t, iw.AddSymbol("cluster"))
	require.NoError(t, iw.AddSymbol("gateway"))
	require.NoError(t, iw.AddSymbol("us"))

	chunks := []ChunkMeta{
		{Checksum: 0, MinTime: 1000, MaxTime: 2000, KB: 10, Entries: 100, StreamID: 42},
		{Checksum: 1, MinTime: 3000, MaxTime: 4000, KB: 15, Entries: 150, StreamID: 7},
		{Checksum: 2, MinTime: 5000, MaxTime: 6000, KB: 20, Entries: 200, StreamID: 0},
	}
	fp := model.Fingerprint(labels.StableHash(series))
	require.NoError(t, iw.AddSeries(0, series, fp, chunks...))
	...
	p, err := ir.Postings("app", nil, "gateway")
	require.NoError(t, err)
	require.True(t, p.Next())

	var ls labels.Labels
	var readChks []ChunkMeta
	_, err = ir.Series(p.At(), 0, math.MaxInt64, &ls, &readChks)
	require.NoError(t, err)
	require.Equal(t, series, ls)
	require.Len(t, readChks, 3)

	require.Equal(t, int64(42), readChks[0].StreamID)
	require.Equal(t, int64(7), readChks[1].StreamID)
	require.Equal(t, int64(0), readChks[2].StreamID)

	for i, chk := range readChks {
		require.Equal(t, chunks[i].Checksum, chk.Checksum)
		require.Equal(t, chunks[i].MinTime, chk.MinTime)
		require.Equal(t, chunks[i].MaxTime, chk.MaxTime)
		require.Equal(t, chunks[i].KB, chk.KB)
		require.Equal(t, chunks[i].Entries, chk.Entries)
	}
}

func TestFormatV3_StreamID_DefaultsToZero(t *testing.T) {
	dir := t.TempDir()
	fn := filepath.Join(dir, IndexFilename)
	...
	series := labels.FromStrings("app", "web")
	require.NoError(t, iw.AddSymbol("app"))
	require.NoError(t, iw.AddSymbol("web"))

	chunks := []ChunkMeta{
		{Checksum: 99, MinTime: 1000, MaxTime: 2000, KB: 5, Entries: 50},
	}
	fp := model.Fingerprint(labels.StableHash(series))
	require.NoError(t, iw.AddSeries(0, series, fp, chunks...))
	...
	require.Equal(t, FormatV3, ir.Version())

	p, err := ir.Postings("app", nil, "web")
	require.NoError(t, err)
	require.True(t, p.Next())
	...
	_, err = ir.Series(p.At(), 0, math.MaxInt64, &ls, &readChks)
	require.NoError(t, err)
	require.Len(t, readChks, 1)
	require.Equal(t, int64(0), readChks[0].StreamID)
}
