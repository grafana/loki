...
func (m *MultiTenantIndex) Stats(ctx context.Context, userID string, from, through model.Time, acc IndexStatsAccumulator, fpFilter index.FingerprintFilter, deletes []*logproto.Delete, matchers ...*labels.Matcher) error {
	return m.idx.Stats(ctx, userID, from, through, acc, fpFilter, deletes, withTenantLabelMatcher(userID, matchers)...)
}

func (m *MultiTenantIndex) Volume(ctx context.Context, userID string, from, through model.Time, acc VolumeAccumulator, fpFilter index.FingerprintFilter, deletes []*logproto.Delete, targetLabels []string, aggregateBy string, matchers ...*labels.Matcher) error {
	return m.idx.Volume(ctx, userID, from, through, acc, fpFilter, deletes, targetLabels, aggregateBy, withTenantLabelMatcher(userID, matchers)...)
}
