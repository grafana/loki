...
func (tm *TableManager) loop() {
	...
	for {
		...
		case <-syncTicker.C:
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error syncing local boltdb files with storage", "err", err)
			}
			...
			err = tm.ensureQueryReadiness()
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error ensuring query readiness of tables", "err", err)
			}
		case <-cacheCleanupTicker.C:
			err := tm.cleanupCache()
			if err != nil {
				level.Error(logutil.Logger).Log("msg", "error cleaning up expired tables", "err", err)
			}
		case <-tm.ctx.Done():
			return
		}
...
func (tm *TableManager) query(ctx context.Context, tableName string, queries []chunk.IndexQuery, callback chunk_util.Callback) error {
	...
	if err != nil {
		if table.Err() != nil {
			...
			level.Error(logutil.Logger).Log("msg", fmt.Sprintf("table %s has some problem, cleaning it up", tableName), "err", table.Err())
...
func (tm *TableManager) getOrCreateTable(spanCtx context.Context, tableName string) *Table {
	...
	if !ok {
		...
		if !ok {
			...
			level.Info(logutil.Logger).Log("msg", fmt.Sprintf("downloading all files for table %s", tableName))
...
func (tm *TableManager) syncTables(ctx context.Context) error {
	...
	level.Info(logutil.Logger).Log("msg", "syncing tables")
...
func (tm *TableManager) cleanupCache() error {
	...
	level.Info(logutil.Logger).Log("msg", "cleaning tables cache")

	for name, table := range tm.tables {
		lastUsedAt := table.LastUsedAt()
		if lastUsedAt.Add(tm.cfg.CacheTTL).Before(time.Now()) {
			level.Info(logutil.Logger).Log("msg", fmt.Sprintf("cleaning up expired table %s", name))
			...
			if err != nil {
				level.Error(logutil.Logger).Log("msg", fmt.Sprintf("failed to remove directory for table %s", name), "err", err)
			}
		}
	}
...
func (tm *TableManager) ensureQueryReadiness() error {
	...
	level.Debug(logutil.Logger).Log("msg", fmt.Sprintf("list of tables required for query-readiness %s", tableNames))
	...
	for _, tableName := range tableNames {
		...
		level.Info(logutil.Logger).Log("msg", "table required for query readiness does not exist locally, downloading it", "table-name", tableName)
...
func (tm *TableManager) loadLocalTables() error {
	...
	for _, fileInfo := range filesInfo {
		...
		level.Info(logutil.Logger).Log("msg", fmt.Sprintf("loading local table %s", fileInfo.Name()))
