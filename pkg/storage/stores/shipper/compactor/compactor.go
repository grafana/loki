...
import (
	...
	deletion_mode "github.com/grafana/loki/pkg/util/deletion"
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet) {
	...
	f.StringVar(&cfg.DeletionMode, "boltdb.shipper.compactor.deletion-mode", "whole-stream-deletion", fmt.Sprintf("(Experimental) Deletion mode. Can be one of %v", strings.Join(deletion_mode.AllModes(), "|")))
...
func (cfg *Config) Validate() error {
	...
	if _, err := deletion_mode.ParseMode(cfg.DeletionMode); err != nil {
		return err
	}
...
func NewCompactor(cfg Config, storageConfig storage.Config, schemaConfig config.SchemaConfig, limits retention.Limits, clientMetrics storage.ClientMetrics, r prometheus.Registerer) (*Compactor, error) {
	...
	mode, err := deletion_mode.ParseMode(cfg.DeletionMode)
	if err != nil {
		return nil, err
	}
...
func (c *Compactor) init(storageConfig storage.Config, schemaConfig config.SchemaConfig, limits retention.Limits, clientMetrics storage.ClientMetrics, r prometheus.Registerer) error {
	...
	if c.cfg.RetentionEnabled {
		...
		switch c.deleteMode {
		case deletion_mode.WholeStreamDeletion, deletion_mode.FilterOnly, deletion_mode.FilterAndDelete:
...
func (c *Compactor) DeleteMode() deletion_mode.Mode {
	return c.deleteMode
}
