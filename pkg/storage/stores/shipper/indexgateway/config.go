...
func (cfg *RingCfg) RegisterFlags(prefix, storePrefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.ReplicationFactor, "replication-factor", 3, "How many index gateway instances are assigned to each tenant. Deprecated: Use -index-gateway.shard-size instead. The shard size is also a per-tenant setting.")
}
