	...
	for _, enc := range blockEncodings {
		...
		for _, tc := range []struct {
			...
			t.Run(desc, func(t *testing.T) {
				...
				querier := NewBlockQuerier(block, HeapAllocator, DefaultMaxPageSize)
...
func TestMergeBuilder(t *testing.T) {
	...
	for i := 0; i < nBlocks; i++ {
		...
		_, err = builder.BuildFrom(itr)
		require.Nil(t, err)
		blocks = append(blocks, NewPeekingIter[*SeriesWithBloom](NewBlockQuerier(NewBlock(reader, NewMetrics(nil)), HeapAllocator, DefaultMaxPageSize)))
	}
...
func TestMergeBuilder_Roundtrip(t *testing.T) {
	...
	mergedBlockQuerier := NewBlockQuerier(NewBlock(reader, NewMetrics(nil)), HeapAllocator, DefaultMaxPageSize)
