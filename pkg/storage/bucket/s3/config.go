...
func (cfg *Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.BoolVar(&cfg.ShuffleAddresses, prefix+"shuffle-addresses", true, "When enabled, pick a random address among those the S3 endpoint resolves to when opening a connection, instead of always preferring the first one returned by the resolver.")
}
