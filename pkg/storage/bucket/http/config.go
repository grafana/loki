...
func (cfg *Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.MaxIdleConns, prefix+"max-idle-connections", 150, "Maximum number of idle (keep-alive) connections across all hosts. 0 means no limit.")
	f.IntVar(&cfg.MaxIdleConnsPerHost, prefix+"max-idle-connections-per-host", 150, "Maximum number of idle (keep-alive) connections to keep per-host. If 0, a built-in default value is used.")
