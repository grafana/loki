...
type PeriodConfig struct {
	...
	IndexType string `yaml:"store" doc:"description=store and object_store below affect which <storage_config> key is used. Which index to use. Either tsdb or boltdb-shipper. Following stores are deprecated: aws, aws-dynamo, cassandra, grpc."`
	// type of object client to use.
	ObjectType  string                   `yaml:"object_store" doc:"description=Which store to use for the chunks. Either aws (alias s3), azure, gcs, alibabacloud, bos, cos, swift, filesystem, or a named_store (refer to named_stores_config). Following stores are deprecated: aws-dynamo, cassandra, grpc."`
...
func validateChunks(cfg PeriodConfig) error {
	...
	switch objectStore {
	case "cassandra", "aws-dynamo", "grpc-store":
