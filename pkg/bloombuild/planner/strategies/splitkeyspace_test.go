...
func Test_gapsBetweenTSDBsAndMetas(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			tsdbs: map[tsdb.SingleTenantTSDBIdentifier]common.ClosableForSeries{
				testutils.TsdbID(0): nil,
			},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(11, 20, []int{0}, nil),
			},
		},
		{
			desc:           "single tsdb",
			...
			metas: []bloomshipper.Meta{
				testutils.GenMeta(4, 8, []int{0}, nil),
			},
			exp: []tsdbGaps{
				{
					tsdbIdentifier: testutils.TsdbID(0),
		...
		},
		{
			...
			tsdbs: map[tsdb.SingleTenantTSDBIdentifier]common.ClosableForSeries{
				testutils.TsdbID(0): nil,
				testutils.TsdbID(1): nil,
			},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(0, 5, []int{0}, nil),
				testutils.GenMeta(6, 10, []int{1}, nil),
			},
			exp: []tsdbGaps{
				{
					tsdbIdentifier: testutils.TsdbID(0),
				...
				{
					tsdbIdentifier: testutils.TsdbID(1),
					gaps: []v1.FingerprintBounds{
						v1.NewBounds(0, 5),
					},
				},
		...
		},
		{
			...
			metas: []bloomshipper.Meta{
				testutils.GenMeta(0, 5, []int{0, 1}, nil),
				testutils.GenMeta(6, 8, []int{1}, nil),
			},
			exp: []tsdbGaps{
				{
					tsdbIdentifier: testutils.TsdbID(0),
...
func Test_blockPlansForGaps(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			ownershipRange: v1.NewBounds(0, 10),
			tsdbs:          []tsdb.SingleTenantTSDBIdentifier{testutils.TsdbID(0)},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(5, 20, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(11, 20)}),
			},
			exp: []blockPlan{
				{
					tsdb: testutils.TsdbID(0),
					gaps: []protos.Gap{
						{
							Bounds: v1.NewBounds(0, 10),
							Series: testutils.GenSeries(v1.NewBounds(0, 10)),
						},
					},
				},
			},
		},
		{
			...
			tsdbs:          []tsdb.SingleTenantTSDBIdentifier{testutils.TsdbID(0)},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(5, 20, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(9, 20)}),
			},
			exp: []blockPlan{
				{
					tsdb: testutils.TsdbID(0),
					gaps: []protos.Gap{
						{
							Bounds: v1.NewBounds(0, 10),
							Series: testutils.GenSeries(v1.NewBounds(0, 10)),
							Blocks: []bloomshipper.BlockRef{testutils.GenBlockRef(9, 20)},
						},
					},
				},
			},
		},
		{
			...
			tsdbs:          []tsdb.SingleTenantTSDBIdentifier{testutils.TsdbID(0)},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(9, 20, []int{0}, []bloomshipper.BlockRef{testutils.GenBlockRef(9, 20)}), // block for same tsdb
				testutils.GenMeta(9, 20, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(9, 20)}), // block for different tsdb
			},
			exp: []blockPlan{
				{
					tsdb: testutils.TsdbID(0),
					gaps: []protos.Gap{
						{
							...
							Series: testutils.GenSeries(v1.NewBounds(0, 8)),
						},
					},
				},
			},
		},
		{
			...
			metas: []bloomshipper.Meta{
				testutils.GenMeta(9, 20, []int{0}, []bloomshipper.BlockRef{testutils.GenBlockRef(9, 20)}), // block for same tsdb
				testutils.GenMeta(5, 20, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(5, 20)}), // block for different tsdb
			},
			exp: []blockPlan{
				{
					tsdb: testutils.TsdbID(0),
					gaps: []protos.Gap{
						{
							Bounds: v1.NewBounds(0, 8),
							Series: testutils.GenSeries(v1.NewBounds(0, 8)),
							Blocks: []bloomshipper.BlockRef{testutils.GenBlockRef(5, 20)},
						},
					},
				},
			},
		},
		{
			desc:           "multi case",
			ownershipRange: v1.NewBounds(0, 10),
			tsdbs:          []tsdb.SingleTenantTSDBIdentifier{testutils.TsdbID(0), testutils.TsdbID(1)}, // generate for both tsdbs
			metas: []bloomshipper.Meta{
				testutils.GenMeta(0, 2, []int{0}, []bloomshipper.BlockRef{
					testutils.GenBlockRef(0, 1),
					testutils.GenBlockRef(1, 2),
				}), // tsdb_0
				testutils.GenMeta(6, 8, []int{0}, []bloomshipper.BlockRef{testutils.GenBlockRef(6, 8)}), // tsdb_0

				testutils.GenMeta(3, 5, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(3, 5)}),   // tsdb_1
				testutils.GenMeta(8, 10, []int{1}, []bloomshipper.BlockRef{testutils.GenBlockRef(8, 10)}), // tsdb_1
			},
			exp: []blockPlan{
				{
					tsdb: testutils.TsdbID(0),
					gaps: []protos.Gap{
						// tsdb (id=0) can source chunks from the blocks built from tsdb (id=1)
						{
							...
							Series: testutils.GenSeries(v1.NewBounds(3, 5)),
							Blocks: []bloomshipper.BlockRef{testutils.GenBlockRef(3, 5)},
						},
						{
							Bounds: v1.NewBounds(9, 10),
							Series: testutils.GenSeries(v1.NewBounds(9, 10)),
							Blocks: []bloomshipper.BlockRef{testutils.GenBlockRef(8, 10)},
						},
					},
				},
				// tsdb (id=1) can source chunks from the blocks built from tsdb (id=0)
				{
					tsdb: testutils.TsdbID(1),
					gaps: []protos.Gap{
						{
							Bounds: v1.NewBounds(0, 2),
							Series: testutils.GenSeries(v1.NewBounds(0, 2)),
						...
						},
						{
							...
							Series: testutils.GenSeries(v1.NewBounds(6, 7)),
							Blocks: []bloomshipper.BlockRef{testutils.GenBlockRef(6, 8)},
						},
					},
				},
			},
		},
		{
			...
			tsdbs:          []tsdb.SingleTenantTSDBIdentifier{testutils.TsdbID(0)},
			metas: []bloomshipper.Meta{
				testutils.GenMeta(9, 20, []int{1}, []bloomshipper.BlockRef{
					testutils.GenBlockRef(1, 4),
					testutils.GenBlockRef(9, 20),
				}), // blocks for first diff tsdb
				testutils.GenMeta(5, 20, []int{2}, []bloomshipper.BlockRef{
					testutils.GenBlockRef(5, 10),
					testutils.GenBlockRef(9, 20), // same block references in prior meta (will be deduped)
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			for _, id := range tc.tsdbs {
				tsdbs[id] = newFakeForSeries(testutils.GenSeries(tc.ownershipRange))
			}
