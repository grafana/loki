...
import (
	...
	"github.com/grafana/loki/v3/pkg/iter/v2"
	"github.com/grafana/loki/v3/pkg/storage/bloom/v1"
...
func CreateTasks(n int, resultsCh chan *protos.TaskResult) []*planner.QueueTask {
	tasks := make([]*planner.QueueTask, 0, n)
	// Enqueue tasks
	for i := 0; i < n; i++ {
		task := planner.NewQueueTask(
