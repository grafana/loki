...
type ringGatherer struct {
	...
	missingInstanceForPartition *prometheus.CounterVec
}
...
func newRingGatherer(
	...
	return &ringGatherer{
		...
		missingInstanceForPartition: promauto.With(reg).NewCounterVec(
			prometheus.CounterOpts{
				Name: "loki_ingest_limits_frontend_missing_instance_for_partition_total",
...
func (g *ringGatherer) doExceedsLimitsRPCs(ctx context.Context, tenant string, streams []*proto.StreamMetadata, partitions map[int32]string, zone string) ([]*proto.ExceedsLimitsResponse, []uint64, error) {
	...
	for _, stream := range streams {
		...
		if !ok {
			g.missingInstanceForPartition.WithLabelValues(zone).Inc()
			continue
		}
