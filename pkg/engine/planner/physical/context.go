...
type Catalog interface {
	...
	ResolveDataObj(Expression, time.Duration) ([]DataObjLocation, [][]int64, [][]int, error)
	ResolveDataObjWithShard(Expression, ShardInfo, time.Duration) ([]DataObjLocation, [][]int64, [][]int, error)
}
...
func (c *Context) ResolveDataObj(selector Expression, rangeInterval time.Duration) ([]DataObjLocation, [][]int64, [][]int, error) {
	return c.ResolveDataObjWithShard(selector, noShard, rangeInterval)
}

func (c *Context) ResolveDataObjWithShard(selector Expression, shard ShardInfo, rangeInterval time.Duration) ([]DataObjLocation, [][]int64, [][]int, error) {
	if c.metastore == nil {
	...
	paths, streamIDs, numSections, err := c.metastore.StreamIDs(c.ctx, c.from.Add(-rangeInterval), c.through, matchers...)
	if err != nil {
