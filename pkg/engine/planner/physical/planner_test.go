...
func (c *catalog) ResolveDataObj(e Expression, rangeInterval time.Duration) ([]DataObjLocation, [][]int64, [][]int, error) {
	return c.ResolveDataObjWithShard(e, noShard, rangeInterval)
}

// ResolveDataObjForShard implements Catalog.
func (c *catalog) ResolveDataObjWithShard(_ Expression, shard ShardInfo, _ time.Duration) ([]DataObjLocation, [][]int64, [][]int, error) {
...
func TestMockCatalog(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run("shard "+tt.shard.String(), func(t *testing.T) {
			paths, streams, sections, _ := catalog.ResolveDataObjWithShard(nil, tt.shard, 0)
