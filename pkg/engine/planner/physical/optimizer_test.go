...
func TestParseKeysPushdownRuleApplies(t *testing.T) {
	...
	t.Run("rule can be called for ParseNode", func(t *testing.T) {
		rule := buildRule()
		...
		// The rule should be callable for a ParseNode (returns false for now as no action taken)
		applied := rule.apply(parseNode)
		require.False(t, applied, "rule should be callable but return false when no changes made")
	})

	t.Run("rule can be added to optimization pipeline", func(t *testing.T) {
		plan := &Plan{}

		// Create an optimization with the parseKeysPushdown rule
		opt := newOptimization("ParseKeysPushdown", plan).withRules(
			&parseKeysPushdown{plan: plan},
		)

		require.NotNil(t, opt, "optimization should be created")
		require.Equal(t, "ParseKeysPushdown", opt.name)
		require.Len(t, opt.rules, 1, "optimization should have one rule")
	})
}
	...
	tests := []struct {
	...
	}{
		{
			...
			buildLogical: func() logical.Value {
				...
				builder = builder.Parse(logical.ParserLogfmt, []string{}, nil)
				return builder.Value()
			},
			expectedKeys: []string{}, // No keys needed
		},
		{
