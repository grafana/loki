...
func NewProjectPipeline(input Pipeline, proj *physical.Projection, evaluator *expressionEvaluator, scope *xcap.Scope) (Pipeline, error) {
	...
	if proj.All && proj.Expand && len(expandExprs) > 0 {
		return newExpandPipeline(expandExprs[0], evaluator, input, scope)
	}

	return nil, errNotImplemented
}

func newKeepPipeline(colRefs []types.ColumnRef, keepFunc func([]types.ColumnRef, *semconv.Identifier) bool, input Pipeline, scope *xcap.Scope) (*GenericPipeline, error) {
...
func newExpandPipeline(expr physical.Expression, evaluator *expressionEvaluator, input Pipeline, scope *xcap.Scope) (*GenericPipeline, error) {
	return newGenericPipelineWithScope(func(ctx context.Context, inputs []Pipeline) (arrow.RecordBatch, error) {
		if len(inputs) != 1 {
			return nil, fmt.Errorf("expected 1 input, got %d", len(inputs))
		}
