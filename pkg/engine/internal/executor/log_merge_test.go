...
func TestDoLogObjectMerge_SortThenMergeFourObjects(t *testing.T) {
	...
	require.Equal(t, 32, totalStreams, "duplicate streams across the four inputs should be deduplicated")
	require.Equal(t, 1280, totalRecords, "sorting and merging must preserve every log record")
}
...
func TestDoLogObjectMerge_DeduplicatesConflictingSourceStreamOrder(t *testing.T) {
	...
	// Both streams have the same schema key, but their local ID order is
	// reversed between objects. The loser tree preserves those source-local
