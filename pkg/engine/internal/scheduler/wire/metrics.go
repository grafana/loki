...
func (m *Metrics) observeHandler(messageType string, outcome metrictimer.Outcome, d time.Duration) {
	m.handlerSeconds.WithLabelValues(messageType, outcome.String()).Observe(d.Seconds())
}
...
func (m *Metrics) observeMessageRoundtrip(kind MessageKind, outcome metrictimer.Outcome, mode sendMode, d time.Duration) {
	m.messageRoundtripSeconds.WithLabelValues(kind.String(), outcome.String(), mode.String()).Observe(d.Seconds())
}
