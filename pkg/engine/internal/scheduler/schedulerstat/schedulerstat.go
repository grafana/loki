...
var (
	...
	// from registration to terminal result.
	TaskStagingDuration = xcap.NewStatisticInt64(statid.SchedulerTaskStagingDuration, "scheduler.task.staging.duration", xcap.AggregationTypeSum)

	// TaskQueueDuration is the time (in nanoseconds) a task spent enqueued before
	// either being assigned to a worker or reaching a terminal result without
	// ever being assigned.
	TaskQueueDuration = xcap.NewStatisticInt64(statid.SchedulerTaskQueueDuration, "scheduler.task.queue.duration", xcap.AggregationTypeSum)

	// TaskExecutionDuration is the time (in nanoseconds) a task spent assigned to
	// a worker before reaching a terminal result. Not recorded for tasks that
	// never reached a worker.
	TaskExecutionDuration = xcap.NewStatisticInt64(statid.SchedulerTaskExecutionDuration, "scheduler.task.execution.duration", xcap.AggregationTypeSum)

	// TaskAssignmentRetries is the number of times a task was requeued after a
	// failed assignment attempt before it reached a terminal result.
	TaskAssignmentRetries = xcap.NewStatisticInt64(statid.SchedulerTaskAssignmentRetries, "scheduler.task.assignment.retries", xcap.AggregationTypeSum)

	// TaskTotalDuration is the total time (in nanoseconds) from scheduler
	// registration to terminal result, regardless of which phases the task
	// passed through.
	//
	...
	// phase.
	TaskTotalDuration = xcap.NewStatisticInt64(statid.SchedulerTaskTotalDuration, "scheduler.task.duration", xcap.AggregationTypeSum)

	// TaskFinishTime is the wall-clock time (Unix nanoseconds) at which the
	// scheduler observed the task reach a terminal result. It is recorded once
	...
	TaskFinishTime = xcap.NewStatisticInt64(statid.SchedulerTaskFinishTime, "scheduler.task.finish.time", xcap.AggregationTypeMax)
)
