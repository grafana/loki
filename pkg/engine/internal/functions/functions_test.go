...
func TestBinaryFunctionRegistry_GetForSignature(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fn, err := Binary.GetForSignature(tt.op, tt.dataType)
...
// TestGetForSignature_EquivalentTypeInstances is a regression test: the
// registry lookup must succeed for any data type instance that is
// structurally equivalent to the registered one, not just for the exact
// singleton used at registration. Interface map keys compare pointer-typed
// Arrow data types by pointer identity, and [arrow.TimestampType] carries
// fields (unit, timezone), so a timestamp type deserialized from IPC or
// constructed inline is a different instance than
// [arrow.FixedWidthTypes.Timestamp_ns]. Keying the registry by [arrow.Type]
// makes the lookup instance-independent.
func TestGetForSignature_EquivalentTypeInstances(t *testing.T) {
	freshInstances := []arrow.DataType{
	...
	for _, dt := range freshInstances {
		t.Run(dt.String(), func(t *testing.T) {
			fn, err := Binary.GetForSignature(types.BinaryOpGt, dt)
			require.NoError(t, err)
			require.NotNil(t, fn)
		})
	}

	fn, err := Unary.GetForSignature(types.UnaryOpNot, &arrow.BooleanType{})
	require.NoError(t, err)
	require.NotNil(t, fn)
}
...
func TestBooleanComparisonFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tt.op, arrow.FixedWidthTypes.Boolean)
			require.NoError(t, err)
...
func TestStringComparisonFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tt.op, arrow.BinaryTypes.String)
			require.NoError(t, err)
...
func TestIntegerComparisonFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tt.op, arrow.PrimitiveTypes.Int64)
			require.NoError(t, err)
...
func TestTimestampComparisonFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tt.op, arrow.FixedWidthTypes.Timestamp_ns)
			require.NoError(t, err)
...
func TestFloat64ComparisonFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tt.op, arrow.PrimitiveTypes.Float64)
			require.NoError(t, err)
...
func TestStringFilterNullAsEmpty(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			fn, err := Binary.GetForSignature(tc.op, arrow.BinaryTypes.String)
			require.NoError(t, err)
...
func TestStringFilterNullAsEmpty_SymmetricLhsScalar(t *testing.T) {
	...
	fn, err := Binary.GetForSignature(types.BinaryOpEq, arrow.BinaryTypes.String)
	require.NoError(t, err)
	...
	fn, err = Binary.GetForSignature(types.BinaryOpMatchRe, arrow.BinaryTypes.String)
	require.NoError(t, err)
...
func TestUnaryNot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fn, err := Unary.GetForSignature(types.UnaryOpNot, arrow.FixedWidthTypes.Boolean)
			require.NoError(t, err)
