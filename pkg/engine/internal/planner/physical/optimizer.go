...
func (p *parallelPushdown) applyParallelization(node Node) bool {
	...
	case *VectorAggregation:
		...
		if !canShardAggregation(n.Operation, rangeAgg.Operation) {
			return false
		}
	...
	case *RangeAggregation:
		...
		if !canShardAggregation(vecAgg.Operation, n.Operation) {
			return false
		}
...
func canShardAggregation(vec types.VectorAggregationType, rng types.RangeAggregationType) bool {
	switch vec {
	case types.VectorAggregationTypeSum:
