...
func (w *Worker) handleConn(ctx context.Context, conn wire.Conn) {
	...
	// TODO(rfratto): As-is, our worker can have up to one Arrow Record per
	// incoming stream sitting in memory, since the peer will immediately accept
	// any the next incoming frame and wait for it to be handled via our
	// Handler.
	//
	// This may be a problem for workers responsible for tasks with a large
	// number of input streams, though it's slightly mitigated if records are
	// processed quickly.
	//
	// Long term, we may want to provide backpressure here and to limit the
	// number of total incoming [wire.StreamDataMessage] messages across all
	// connected peers.
	peer := &wire.Peer{
		Logger: logger,
		Conn:   conn,

		// We don't want each data plane connections to have its own buffer,
		// otherwise the problem listed above gets even worse. If we do have
		// buffering, it should be at the thread level.
		Buffer: 0,
