...
func (t *thread) runJob(ctx context.Context, job *threadJob) {
	...
	_, drainErr := t.drainPipeline(ctx, taskType, pipeline, sinksForJob(job), job.Task.SinkRouting, logger)
...
func (t *thread) drainPipeline(ctx context.Context, taskType string, pipeline executor.Pipeline, sinks []recordSink, routing *workflow.SinkRouting, logger log.Logger) (int, error) {
