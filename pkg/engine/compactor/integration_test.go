...
func TestCoordinator_EndToEnd(t *testing.T) {
	...
	_, _, _, runErr := c.runTenantCycle(ctx, "acme", window, preCycle1)
	...
	if len(indexesC2) > 1 {
		_, _, _, runErr := c.runTenantCycle(ctx, "acme", window, indexesC2)
		require.NoError(t, runErr)
	...
	for i := range 5 {
		...
		_, _, _, runErr := c.runTenantCycle(ctx, "acme", window, acmeIdx)
		require.NoError(t, runErr)
