...
func SampleJsoniterEncode(ptr unsafe.Pointer, stream *jsoniter.Stream) {
	...
	if legacySample.Histogram != nil {
		histogramBuckets := make(model.HistogramBuckets, 0, len(legacySample.Histogram.Buckets))
		for i, b := range legacySample.Histogram.Buckets {

			histogramBuckets[i] = &model.HistogramBucket{Lower: model.FloatString(b.Lower), Upper: model.FloatString(b.Upper), Count: model.FloatString(b.Count)}

		}
		sampleHistogram := model.SampleHistogram{Count: model.FloatString(legacySample.Histogram.Count), Sum: model.FloatString(legacySample.Histogram.Sum), Buckets: histogramBuckets}
		stream.WriteVal(sampleHistogram)
