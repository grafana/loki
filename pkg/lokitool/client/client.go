...
// LokiClient is used to get and load rules
...
func (r *LokiClient) doRequest(path, method string, payload []byte) (*http.Response, error) {
	...
	if (r.user != "" || r.key != "") && r.authToken != "" {
		...
		log.WithFields(log.Fields{
			...
			"method": req.Method,
			"error":  err,
		}).Errorln("error during request to loki api")
		return nil, err
	}
	...
	log.WithFields(log.Fields{
		"url":    req.URL.String(),
		"method": req.Method,
	}).Debugln("sending request to loki api")
