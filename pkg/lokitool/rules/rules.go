...
func (r RuleNamespace) LintExpressions() (int, int, error) {
	...
	for i, group := range r.Groups {
		for j, rule := range group.Rules {
			log.WithFields(log.Fields{"rule": getRuleName(rule)}).Debug("linting LogQL")
			exp, err := syntax.ParseExpr(rule.Expr.Value)
			if err != nil {
...
func (r RuleNamespace) AggregateBy(label string, applyTo func(group rulefmt.RuleGroup, rule rulefmt.RuleNode) bool) (int, int, error) {
