...
var (
	...
	ErrInvalidProducerMaxInflightRequestsPerBroker = errors.New("the configured producer max inflight requests per broker must be greater than or equal to 1")
	ErrInvalidProducerLinger                       = errors.New("the configured producer linger must be greater than or equal to 0")
)
...
type Config struct {
	...
	ProducerMaxInflightRequestsPerBroker int           `yaml:"producer_max_inflight_requests_per_broker"`
	ProducerLinger                       time.Duration `yaml:"producer_linger"`
...
func (cfg *Config) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	...
	f.IntVar(&cfg.ProducerMaxInflightRequestsPerBroker, prefix+".producer-max-inflight-requests-per-broker", 20,
		"The maximum number of Produce requests the producer can have in-flight per Kafka broker at any given time. "+
			"The product of this value and -"+prefix+".producer-linger should exceed the maximum Produce latency expected "+
			"from the Kafka backend in steady state. "+
			"If the backend takes longer than that to process a Produce request, the client will buffer data and stop "+
			"issuing new Produce requests until some in-flight ones complete, which surfaces as added latency to callers.")
	f.DurationVar(&cfg.ProducerLinger, prefix+".producer-linger", 50*time.Millisecond,
		"How long the producer waits, buffering records for the same partition, before sending a Produce request. "+
			"The product of this value and -"+prefix+".producer-max-inflight-requests-per-broker should exceed the maximum "+
			"Produce latency expected from the Kafka backend in steady state. "+
			"If the backend takes longer than that to process a Produce request, the client will buffer data and stop "+
			"issuing new Produce requests until some in-flight ones complete, which surfaces as added latency to callers.")
...
func (cfg *Config) Validate() error {
	...
	if cfg.ProducerMaxInflightRequestsPerBroker < 1 {
		return ErrInvalidProducerMaxInflightRequestsPerBroker
	}
	if cfg.ProducerLinger < 0 {
		return ErrInvalidProducerLinger
	}
