...
// One entry can widen the length varint of its scope and of its resource at once, so this checks
// the bookkeeping is exact: the growth predicted is the growth that happens, and the size carried
// is what Size() reports.
func TestInternalBatchSizeTracksTheProto(t *testing.T) {
	// sizingStream is shaped so that walking it reaches every branch of the size arithmetic.
	sizingStream := func() logproto.InternalStreamAdapter {
		// These set how many entries or scopes it takes to grow past a boundary.
		const (
			lineLen = 200
			attrLen = 100
		)
		entry := func() push.Entry {
			// No structured metadata: an entry's size is only an input to the arithmetic.
			return internalEntry(strings.Repeat("x", lineLen))
		}
		scopeAttrs := internalAttrs("scope_key", strings.Repeat("s", attrLen))
		resourceAttrs := internalAttrs("resource_key", strings.Repeat("r", attrLen))

		grownScope := func() logproto.ScopeLogs {
			scope := logproto.ScopeLogs{Attrs: scopeAttrs}
			for sovPush(uint64(scope.Size())) < 3 {
				scope.Entries = append(scope.Entries, entry())
			}
			return scope
		}

		// Scopes long enough to need a three-byte length, so both a scope and its resource pass the
		// one and two byte boundaries on an add that appends to a scope the batch already holds.
		grownResource := logproto.ResourceLogs{
			Attrs:     resourceAttrs,
			ScopeLogs: []logproto.ScopeLogs{grownScope(), grownScope()},
		}

		// A single scope, so the resource after this one opens with the scope index still zero: the
		// only case where a resource changes and the scope index does not.
		oneScopeResource := logproto.ResourceLogs{
			Attrs:     resourceAttrs,
			ScopeLogs: []logproto.ScopeLogs{{Attrs: scopeAttrs, Entries: []push.Entry{entry()}}},
		}

		// Scopes holding one entry each, enough of them to need a three byte length. Every entry
		// opens a scope, so this resource passes a boundary on an add that opens one.
		manyScopesResource := logproto.ResourceLogs{Attrs: resourceAttrs}
		for sovPush(uint64(manyScopesResource.Size())) < 3 {
			manyScopesResource.ScopeLogs = append(manyScopesResource.ScopeLogs,
				logproto.ScopeLogs{Attrs: scopeAttrs, Entries: []push.Entry{entry()}})
		}

		return logproto.InternalStreamAdapter{
			Labels:       `{app="a", env="prod"}`,
			Hash:         12345,
			ResourceLogs: []logproto.ResourceLogs{grownResource, oneScopeResource, manyScopesResource},
		}
	}

	source := sizingStream()

	batch := newInternalBatch(source.Labels, source.Hash)
	require.Equal(t, batch.stream.Size(), batch.size, "a fresh batch")

	for place, entry := range placements(&source) {
		entryLen := entry.Size()
		before := batch.stream.Size()
		predicted := batch.growthFor(place, entryLen)
		batch.add(place, entry, entryLen)

		require.Equal(t, before+predicted, batch.stream.Size(),
			"growth predicted for an entry of resource %d scope %d", place.resourceIdx, place.scopeIdx)
		require.Equal(t, batch.stream.Size(), batch.size,
			"size tracked after an entry of resource %d scope %d", place.resourceIdx, place.scopeIdx)
	}
}
