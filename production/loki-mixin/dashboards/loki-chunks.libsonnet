  ...
  grafanaDashboards+: {
    ...
    'loki-chunks.json': {
                        ...
                        .addRow(
                          ...
                          .addPanel(
                            $.panel('Queue Length') +
                            $.queryPanel('cortex_ingester_flush_queue_length{%(label)s} or loki_ingester_flush_queue_length{%(label)s}' % {label: dashboards['loki-chunks.json'].labelsSelector}, '{{pod}}'),
                          )
                          .addPanel(
                            $.panel('Flush Rate') +
