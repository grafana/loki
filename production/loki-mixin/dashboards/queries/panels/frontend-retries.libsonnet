...
local targets = std.flattenArrays(
    [
      [
        lib.query.prometheus.new(
          ...
          expr=shared.queries.queryFrontend['retries_p' + p],
          params={
            refId: 'p' + p,
            legendFormat: 'p' + p,
          }
  ...
  + [
    lib.query.prometheus.new(
      datasource=common.variables.metrics_datasource.name,
      expr=shared.queries.queryFrontend.retries_average,
      params={
        refId: 'avg',
        legendFormat: 'avg',
      }
...
lib.panels.timeSeries.seconds({
  title: 'Retries',
  description: |||
    The number of times a query has been retried by the Query Frontend.
  |||,
  datasource: common.variables.metrics_datasource.name,
  targets: targets,
  fillOpacity: 10,
