...
(import 'dashboard-utils.libsonnet') {
  ...
  local ingester_zone_job_matcher = if $._config.ssd.enabled then '%s-write' % $._config.ssd.pod_prefix_matcher else 'ingester-zone.+',
  ...
  grafanaDashboards+::
    {
      'loki-writes-resources.json':
        ...
        .addRow(
          ...
          .addPanel(
            $.goHeapInUsePanel('Memory (go heap inuse), ingester', ingester_job_matcher),
          )
          .addPanel(
            $.panel('Disk Writes') +
            $.queryPanel(
