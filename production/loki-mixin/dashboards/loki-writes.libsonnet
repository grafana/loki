  ...
  grafanaDashboards+: {
    ...
    'loki-writes.json': {
                        ...
                        .addRow(
                          ...
                          .addPanel(
                            $.newQueryPanel('Data Acceptance Rate', '%') +
                            $.queryPanel('
                              (
                                sum(rate(loki_distributor_bytes_received_total{%s}[$__rate_interval])) - 
                                (sum(rate(loki_discarded_bytes_total{%s}[$__rate_interval])) or vector(0))
                              ) / sum(rate(loki_distributor_bytes_received_total{%s}[$__rate_interval])) * 100
                            ' % [dashboards['loki-writes.json'].distributorSelector, dashboards['loki-writes.json'].distributorSelector, dashboards['loki-writes.json'].distributorSelector], 'acceptance rate') + {
                              stack: true,
                              yAxes: [
