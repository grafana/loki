    ...
    'promtail.json': {
                     ...
                     .addRow(
                       ...
                       .addPanel(
                         $.panel('Bps') +
                         $.queryPanel(
                           'sum(rate(promtail_read_bytes_total{%s}[1m]))' % dashboards['promtail.json'].labelsSelector,
                           'logs read',
                         ) +
                         { yaxes: $.yaxes('Bps') },
                       )
                       .addPanel(
                         $.panel('Lines') +
                         $.queryPanel(
                           'sum(rate(promtail_read_lines_total{%s}[1m]))' % dashboards['promtail.json'].labelsSelector,
                           'lines read',
                         ),
                       )
                     )
