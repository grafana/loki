  ...
  grafanaDashboards+: {
    ...
    'loki-operational.json': {
                               ...
                               podMatchers:: {
                                 ...
                                 distributor: if $._config.meta_monitoring.enabled
                                 then [utils.selector.re('pod', '((enterprise-logs|loki)-)?(distributor|%s-write|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                                 else [utils.selector.re('pod', '%s' % (if $._config.ssd.enabled then '%s-write.*' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?distributor.*'))],
                                 ingester: if $._config.meta_monitoring.enabled
                                 then [utils.selector.re('pod', '((enterprise-logs|loki)-)?(partition-ingester.*|ingester.*|%s-write|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                                 else [utils.selector.re('pod', '%s' % (if $._config.ssd.enabled then '%s-write.*' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?(ingester.*|partition-ingester.*)'))],
                                 querier: if $._config.meta_monitoring.enabled
                                 then [utils.selector.re('pod', '((enterprise-logs|loki)-)?(querier|%s-read|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                                 else [utils.selector.re('pod', '%s' % (if $._config.ssd.enabled then '%s-read.*' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?querier.*'))],
