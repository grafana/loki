local raw = (import './dashboard-stream-sharding.json');
...
// 2. Copy the JSON into `dashboard-stream-sharding.json`
// 3. Delete the `id` and `templating` fields from the JSON
(import 'dashboard-utils.libsonnet') {

  grafanaDashboards+:: {
    'loki-stream-sharding.json':
      raw
      {
      ...
      + $.dashboard('Loki / Stream Sharding', uid='stream-sharding')
        .addCluster()
        .addNamespace()
        .addLog()
        .addTag()
