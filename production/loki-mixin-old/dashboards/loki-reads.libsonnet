  ...
  grafanaDashboards+: {
    ...
    'loki-reads.json': {
                         ...
                         matchers:: {
                           ...
                           then [utils.selector.re('job', '($namespace)/((enterprise-logs|loki)-)?(query-frontend|%s-read|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-read' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?query-frontend'))],
                           querier: if $._config.meta_monitoring.enabled
                           then [utils.selector.re('job', '($namespace)/((enterprise-logs|loki)-)?(querier|%s-read|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-read' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?querier'))],
                           ...
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-write' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?(partition-ingester-.*|ingester-zone.*)'))],
                           querierOrIndexGateway: if $._config.meta_monitoring.enabled
                           then [utils.selector.re('job', '($namespace)/((enterprise-logs|loki)-)?(querier|index-gateway|%s-read|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-read' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?(querier|index-gateway)'))],
                           indexGateway: if $._config.meta_monitoring.enabled
                           then [utils.selector.re('job', '($namespace)/((enterprise-logs|loki)-)?(index-gateway|%s-backend|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-backend' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?index-gateway'))],
                           bloomGateway: if $._config.meta_monitoring.enabled
                           then [utils.selector.re('job', '($namespace)/((enterprise-logs|loki)-)?(bloom-gateway|%s-backend|loki-single-binary)' % $._config.ssd.pod_prefix_matcher)]
                           else [utils.selector.re('job', '($namespace)/%s' % (if $._config.ssd.enabled then '%s-backend' % $._config.ssd.pod_prefix_matcher else '((enterprise-logs|loki)-)?bloom-gateway'))],
                         },
