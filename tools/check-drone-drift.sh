...
modified_drone_jsonnet=$(git diff "${TARGET_BRANCH}"..."${BRANCH}" -- "${DRONE_JSONNET_FILE}" | wc -l)
  if [[ "${modified_drone_jsonnet}" -gt "0" ]]; then
    modified_drone_config=$(git diff "${TARGET_BRANCH}"..."${BRANCH}" -- "${DRONE_CONFIG_FILE}" | wc -l)
