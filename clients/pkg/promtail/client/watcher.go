...
func NewWatcher(metrics *WatcherMetrics, readerMetrics *wal.LiveReaderMetrics, logger log.Logger, name string, writer WriteTo, dir string, sendExemplars, sendHistograms bool) *Watcher {
	if logger == nil {
...
func (w *Watcher) Run() error {
	...
	lastCheckpoint, checkpointIndex, err := wal.LastCheckpoint(w.walDir)
...
func (w *Watcher) watch(segmentNum int, tail bool) error {
	...
	reader := wal.NewLiveReader(w.logger, w.readerMetrics, segment)
...
func (w *Watcher) readSegment(r *wal.LiveReader, segmentNum int, tail bool) error {
	var (
		dec              record.Decoder
