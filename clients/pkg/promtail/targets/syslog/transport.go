...
func (t *TCPTransport) handleConnection(cn net.Conn) {
	...
	err := syslogparser.ParseStream(t.config.IsRFC3164Message(), t.config.DefaultToCurrentYear, c, func(result *syslog.Result) {
...
func (t *UDPTransport) handleRcv(c *ConnPipe) {
	...
	for {
		...
		err = syslogparser.ParseStream(t.config.IsRFC3164Message(), t.config.DefaultToCurrentYear, r, func(result *syslog.Result) {
			if err := result.Error; err != nil {
				t.handleMessageError(err)
			} else {
				t.handleMessage(lbs.Copy(), result.Message)
