...
import (
	...
	wal2 "github.com/grafana/loki/pkg/util/wal"
)
...
func ReadWAL(dir string) ([]api.Entry, error) {
	reader, close, err := wal2.NewWalReader(dir, -1)
	if err != nil {
		return nil, err
	}
	...
	for reader.Next() {
		var walRec = ingester.WALRecord{}
		bytes := reader.Record()
		err = ingester.DecodeWALRecord(bytes, &walRec)
		if err != nil {
