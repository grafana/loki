...
func Test_loki_sendRecord(t *testing.T) {
	...
	tests := []struct {
		...
		{"labelmap", &config{labelMap: map[string]any{"bar": "other"}, lineFormat: jsonFormat, removeKeys: []string{"bar", "error"}}, simpleRecordFixture, []util.Entry{{Labels: model.LabelSet{"other": "500"}, Entry: logproto.Entry{Line: `{"foo":"bar"}`, Timestamp: now}}}, false},
		...
		{"JSON inner string escaping", &config{removeKeys: []string{"kubernetes"}, labelMap: map[string]any{"kubernetes": map[string]any{"annotations": map[string]any{"kubernetes.io/psp": "label"}}}, lineFormat: jsonFormat}, nestedJSONFixture, []util.Entry{{Labels: model.LabelSet{"label": "test"}, Entry: logproto.Entry{Line: `{"log":"\tstatus code: 403, request id: b41c1ffa-c586-4359-a7da-457dd8da4bad\n"}`, Timestamp: now}}}, false},
	}
