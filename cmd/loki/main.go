...
func main() {
	...
	if reflect.DeepEqual(&config.Server.LogLevel, &logging.Level{}) {
		_ = level.Error(util_log.Logger).Log("msg", "invalid log level")
		os.Exit(1)
	}
	...
	err := config.Validate()
	if err != nil {
		_ = level.Error(util_log.Logger).Log("msg", "validating config", "err", err.Error())
		os.Exit(1)
	}

	if config.verifyConfig {
		_ = level.Info(util_log.Logger).Log("msg", "config is valid")
		os.Exit(0)
	}
	...
	if config.Tracing.Enabled {
		...
		if err != nil {
			_ = level.Error(util_log.Logger).Log("msg", "error in initializing tracing. tracing will not be enabled", "err", err)
		}
		defer func() {
			if trace != nil {
				if err := trace.Close(); err != nil {
					_ = level.Error(util_log.Logger).Log("msg", "error closing tracing", "err", err)
				}
			}
	...
	_ = level.Info(util_log.Logger).Log("msg", "Starting Loki", "version", version.Info())
