...
func newQueryClient(app *kingpin.Application) client.Client {
	...
	app.Flag("cache-control", "adds Cache-Control http header to API requests. Setting value `no-store` will not use results cache. Can also be set using LOKI_CACHE_CONTROL env var.").Default("").Envar("LOKI_CACHE_CONTROL").StringVar(&client.CacheControl)
