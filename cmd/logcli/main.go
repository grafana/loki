...
func newQuery(instant bool, cmd *kingpin.CmdClause) *query.Query {
	...
	cmd.Flag("remote-schema", "Execute the current query using a remote schema retrieved using the configured storage in the given Loki configuration file.").Default("false").BoolVar(&q.RemoteSchema)
