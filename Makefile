...
ifeq ($(UNAME_S),Linux)
  GO_FLAGS := -ldflags "-extldflags \"-static\" -s -w $(GO_LDFLAGS)" -tags netgo,slicelabels,localvalidationscheme
else
  GO_FLAGS := -ldflags "-s -w $(GO_LDFLAGS)" -tags netgo,slicelabels,localvalidationscheme
endif
DYN_GO_FLAGS       := -ldflags "-s -w $(GO_LDFLAGS)" -tags netgo,slicelabels,localvalidationscheme
...
DEBUG_GO_FLAGS     := -gcflags "all=-N -l" -ldflags "-extldflags \"-static\" $(GO_LDFLAGS)" -tags netgo,slicelabels,localvalidationscheme
DEBUG_DYN_GO_FLAGS := -gcflags "all=-N -l" -ldflags "$(GO_LDFLAGS)" -tags netgo,slicelabels,localvalidationscheme
...
PROMTAIL_GO_TAGS = promtail_journal_enabled slicelabels localvalidationscheme
endif
...
ifeq ($(SKIP_ARM),true)
	...
	CGO_ENABLED=1 $(CGO_GOX)  -tags promtail_journal_enabled,slicelabels,localvalidationscheme -osarch="linux/amd64" ./clients/cmd/promtail
else
	...
	PKG_CONFIG_PATH="/usr/lib/aarch64-linux-gnu/pkgconfig" CC="aarch64-linux-gnu-gcc" $(CGO_GOX)  -tags promtail_journal_enabled,slicelabels,localvalidationscheme -osarch="linux/arm64" ./clients/cmd/promtail
	PKG_CONFIG_PATH="/usr/lib/arm-linux-gnueabihf/pkgconfig" CC="arm-linux-gnueabihf-gcc" $(CGO_GOX)  -tags promtail_journal_enabled,slicelabels,localvalidationscheme -osarch="linux/arm" ./clients/cmd/promtail
	CGO_ENABLED=1 $(CGO_GOX)  -tags promtail_journal_enabled,slicelabels,localvalidationscheme -osarch="linux/amd64" ./clients/cmd/promtail
endif
...
ifeq ($(BUILD_IN_CONTAINER),true)
	...
	GO111MODULE=on golangci-lint run -v --timeout 15m --build-tags linux,promtail_journal_enabled,slicelabels,localvalidationscheme
	GOFLAGS="-tags=linux,promtail_journal_enabled,slicelabels,localvalidationscheme" faillint -paths \
...
test-integration:
	$(GOTEST) -count=1 -v -tags=integration,slicelabels,localvalidationscheme -timeout 15m ./integration
