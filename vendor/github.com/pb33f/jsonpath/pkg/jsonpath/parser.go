...
func newParserPrivate(tokenizer *token.Tokenizer, tokens []token.TokenInfo, opts ...config.Option) *JSONPath {
	return newParserPrivateWithConfig(tokenizer, tokens, config.New(opts...))
}

func newParserPrivateWithConfig(tokenizer *token.Tokenizer, tokens []token.TokenInfo, cfg config.Config) *JSONPath {
...
func (p *JSONPath) parseSegment() (*segment, error) {
	...
	if currentToken.Token == token.RECURSIVE {
		...
		if currentToken.Token == token.CHILD {
			if config.SpectralCompatibilityEnabled(p.config) && p.current+1 < len(p.tokens) && p.tokens[p.current+1].Token == token.PROPERTY_NAME {
				p.current += 2
				return &segment{kind: segmentKindRecursivePropertyName}, nil
			}
			p.current++
		}
		child, err := p.parseInnerSegment()
...
func (p *JSONPath) parseSelector() (retSelector *selector, err error) {
	...
	if p.tokens[p.current].Token == token.STRING_LITERAL {
		...
		return &selector{
			kind:         selectorSubKindArrayIndex,
			index:        i,
			jsonPathPlus: p.config.JSONPathPlusEnabled() && p.filterDepth == 0,
			spectral:     config.SpectralCompatibilityEnabled(p.config),
		}, nil
...
func (p *JSONPath) parseFilterSelector() (*selector, error) {
	...
	if config.SpectralCompatibilityEnabled(p.config) {
		expr, err := p.parseSpectralExpression()
		if err != nil {
			return nil, err
		}
		return &selector{kind: selectorSubKindFilter, filter: filterSelector{spectralExpression: expr}}, nil
	}
	...
	return &selector{kind: selectorSubKindFilter, filter: filterSelector{expression: expr}}, nil
}
