...
Database model structs use decoders generated by the `maxminddb-gen` tool from
`maxminddb-golang`. This is transparent to library users and improves lookup
performance while preserving the decoded results.

When changing `models.go`, run `go generate ./...` and commit the updated
`models_maxminddb.go`. The generator version is pinned in `go.mod`, and CI
checks that generated files are current.
