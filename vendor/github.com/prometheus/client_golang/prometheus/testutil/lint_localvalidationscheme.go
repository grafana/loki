...
func CollectAndLint(c prometheus.Collector, scheme model.ValidationScheme, metricNames ...string) ([]promlint.Problem, error) {
	reg := prometheus.NewPedanticRegistry()
	if err := reg.Register(c); err != nil {
		return nil, fmt.Errorf("registering collector failed: %w", err)
	}
	return GatherAndLint(reg, scheme, metricNames...)
}
...
func GatherAndLint(g prometheus.Gatherer, scheme model.ValidationScheme, metricNames ...string) ([]promlint.Problem, error) {
	got, err := g.Gather()
	if err != nil {
	...
	return promlint.NewWithMetricFamilies(got).Lint(scheme)
}
