...
The configuration can be overridden by the [WithEndpoint], [WithEndpointURL], [WithInsecure], and [WithGRPCConn] options.

OTEL_EXPORTER_OTLP_INSECURE, OTEL_EXPORTER_OTLP_LOGS_INSECURE (default: "false") -
setting "true" disables client transport security for the exporter's gRPC connection.
You can use this only when an endpoint is provided without a scheme.
OTEL_EXPORTER_OTLP_LOGS_INSECURE takes precedence over OTEL_EXPORTER_OTLP_INSECURE.
The configuration can be overridden by the [WithInsecure] and [WithGRPCConn] options.
...
The configuration can be overridden by the [WithCompressor] and [WithGRPCConn] options.
...
The configuration can be overridden by the [WithTLSCredentials] and [WithGRPCConn] options.
